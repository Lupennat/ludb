import { PostgresConfig } from '../../../src/types/config';
import {
  config,
  currentGenericDB,
  currentPostgresDB,
...
maybe('Postgres Schema Builder', () => {
  ...
  (config[currentDB] as PostgresConfig).search_path =
    'public,test_schema_private';
  ...
  beforeAll(async () => {
    await DB.connection(currentDB).statement(
      'create schema if not exists test_schema_private',
    );
  });

  afterAll(async () => {
    await DB.connection(currentDB).statement(
      'drop table if exists public.test_schema_table',
    );
    await DB.connection(currentDB).statement(
      'drop table if exists private.test_schema_table',
    );

    await DB.connection(currentDB).statement(
      'drop view if exists public.test_schema_foo',
    );
    await DB.connection(currentDB).statement(
      'drop view if exists private.test_schema_foo',
    );

    await DB.connection(currentDB).statement(
      'drop schema test_schema_private cascade',
    );
  ...
  it('Works Drop All Tables On All Schemas', async () => {
    ...
    expect(
      await Schema.hasTable('test_schema_private.test_schema_table'),
    ).toBeFalsy();
  });

  it('Works Drop All Tables Uses Dont Drop Config On All Schemas', async () => {
    (config[currentDB] as PostgresConfig).dont_drop = [
      'spatial_ref_sys',
      'test_schema_table',
    ];
    await purge();
  ...
  it('Works Drop All Views On All Schemas', async () => {
    await Schema.createView(
      'create view public.test_schema_foo (id) as select 1',
    );
    await Schema.createView(
    ...
    expect(
      await Schema.hasView('test_schema_private.test_schema_foo'),
    ).toBeFalsy();
  });
  ...
  it('Works Add Table Comment On New Table', async () => {
    ...
    const table = (await Schema.getTables()).find(
      (table) => table.name === 'test_schema_posts',
    );
  ...
  it('Works Get All Tables And Column Listing', async () => {
    ...
    expect(
      (await Schema.getColumns('test_schema_users')).map(
        (column) => column.name,
      ),
    ).toEqual(['id', 'name', 'age', 'color']);
  ...
  it('Works Get All Views', async () => {
    ...
    const view = (await Schema.getViews()).find(
      (table) => table.name === 'test_schema_users_view',
    );
