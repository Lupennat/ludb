...
import { currentGenericDB, DB } from './fixtures/config';
...
describe('QueryBuilder', () => {
      ...
      .insert([
        {
          title: 'Foo Post',
          content: 'Lorem Ipsum.',
          created_at: '2017-11-12 13:14:15',
        },
        {
          title: 'Bar Post',
          content: 'Lorem Ipsum.',
        ...
        },
        {
          title: 'Foo Post',
          content: 'Lorem Ipsum.',
          created_at: '2020-08-09 10:07:08',
        },
        {
          title: 'Bat Post',
          content: 'Lorem Ipsum.',
          created_at: '2019-03-15 02:01:00',
        },
      ]);
  ...
  it('Works Increment', async () => {
    ...
    let rows = await DB.connection(currentGenericDB)
      .table('test_query_builder_accounting')
      .get();
    ...
    rows = await DB.connection(currentGenericDB)
      .table('test_query_builder_accounting')
      .get();
  ...
  it('Works Sole Fails For Multiple Records', async () => {
    await expect(
      DB.connection(currentGenericDB)
        .table('test_query_builder_posts')
  ...
  it('Works From With Alias', async () => {
    expect(4).toBe(
      (
        await DB.connection(currentGenericDB)
          .table('test_query_builder_posts', 'alias')
          .select('alias.*')
          .get()
      ).length,
    );
