...
describe('QueryBuilder Order-Group', () => {
  it('Works Group Bys', () => {
    ...
    expect('select * from "users" group by "id", "email"').toBe(
      builder.toSql(),
    );
    ...
    expect('select * from "users" group by DATE(created_at)').toBe(
      builder.toSql(),
    );
    ...
    expect('select * from "users" group by DATE(created_at), ? DESC').toBe(
      builder.toSql(),
    );
    ...
    expect([
      ...
      'groupByRawBinding',
      'havingRawBinding',
    ]).toEqual(builder.getBindings());
  });

  it('Works Order Bys', () => {
    let builder = getBuilder();
    builder.select('*').from('users').orderBy('email').orderBy('age', 'desc');
    expect('select * from "users" order by "email" asc, "age" desc').toBe(
      builder.toSql(),
    );
  ...
  it('Works Latest', () => {
    ...
    expect('select * from "users" order by "created_at" desc').toBe(
      builder.toSql(),
    );

    builder = getBuilder();
    builder.select('*').from('users').latest().limit(1);
    expect('select * from "users" order by "created_at" desc limit 1').toBe(
      builder.toSql(),
    );

    builder = getBuilder();
    builder.select('*').from('users').latest('updated_at');
    expect('select * from "users" order by "updated_at" desc').toBe(
      builder.toSql(),
    );
  });
  ...
  it('Works Oldest', () => {
    ...
    expect('select * from "users" order by "created_at" asc').toBe(
      builder.toSql(),
    );

    builder = getBuilder();
    builder.select('*').from('users').oldest().limit(1);
    expect('select * from "users" order by "created_at" asc limit 1').toBe(
      builder.toSql(),
    );

    builder = getBuilder();
    builder.select('*').from('users').oldest('updated_at');
    expect('select * from "users" order by "updated_at" asc').toBe(
      builder.toSql(),
    );
  });
  ...
  it('Works Order Bys Sqlserver', () => {
    ...
    expect(
      'select * from [users] order by [email] desc offset 25 rows fetch next 10 rows only',
    ).toBe(builder.toSql());
  });
  ...
  it('Works Reorder', () => {
    ...
    builder.reorder();
    expect('(select * from "first") union (select * from "second")').toBe(
      builder.toSql(),
    );
  ...
  it('Works Sqlserver Limits And Offsets', () => {
    ...
    expect('select * from [users] order by [email] desc offset 10 rows').toBe(
      builder.toSql(),
    );
