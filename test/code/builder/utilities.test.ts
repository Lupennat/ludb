...
describe('QueryBuilder Utilities', () => {
  ...
  it('Works QueryBuilder Cache Set Cache to Connection Session', () => {
    ...
    const builder = new QueryBuilder(session);
    expect(
      builder.cache({ cache: 1000, key: 'test', options: { a: true } }),
    ).toBeInstanceOf(QueryBuilder);
    expect(spiedCache).toHaveBeenCalledTimes(1);
    expect(spiedCache).toHaveBeenCalledWith({
      cache: 1000,
      key: 'test',
      options: { a: true },
    });
  ...
  it('Works When Callback With Default', () => {
    ...
    builder
      .select('*')
      .from('users')
      .when<string>('truthy', callback, defaultCB)
      .where('email', 'foo');
    expect(builder.toSql()).toBe(
      'select * from "users" where "id" = ? and "email" = ?',
    );
    ...
    builder
      .select('*')
      .from('users')
      .when<number>(0, callback, defaultCBNoReturn)
      .where('email', 'foo');
    expect(builder.toSql()).toBe(
      'select * from "users" where "id" = ? and "email" = ?',
    );
  ...
  it('Works Unless Callback With Default', () => {
    ...
    builder
      .select('*')
      .from('users')
      .unless<number>(0, callback, defaultCB)
      .where('email', 'foo');
    expect(builder.toSql()).toBe(
      'select * from "users" where "id" = ? and "email" = ?',
    );
    ...
    builder
      .select('*')
      .from('users')
      .unless<string>('truthy', callback, defaultCBNoReturn)
      .where('email', 'foo');
    expect(builder.toSql()).toBe(
      'select * from "users" where "id" = ? and "email" = ?',
    );
  ...
  it('Works Preserved Are Applied By Delete', async () => {
    const builder = getBuilder();
    const spiedDelete = jest
      .spyOn(builder.getConnection(), 'delete')
      .mockImplementation();
    builder.beforeQuery(function (builder) {
      builder.from('users');
    });
  ...
  it('Works Preserved Are Applied By Truncate', async () => {
    ...
    const spiedStatement = jest
      .spyOn(builder.getConnection(), 'statement')
      .mockImplementation();
    builder.beforeQuery(function (builder) {
      builder.from('users');
    });
  ...
  it('Works Preserved Are Applied By Exists', async () => {
    ...
    expect(spiedSelect).toHaveBeenCalledWith(
  ...
  it('Works Merge Wheres Can Merge Wheres And Bindings', () => {
    ...
    builder.where('foo', 'bar');
    builder.mergeWheres(
      toMerge.getRegistry().wheres,
      toMerge.getRawBindings().where,
    );
    expect(['foo', 'bar']).toEqual(
      builder.getRegistry().wheres.map((where) => (where as WhereBasic).column),
    );
  ...
  it('Works Clone', () => {
    ...
    expect(
      'select * from "users" where "email" = ? order by "foo" desc, "baz" desc',
    ).toBe(clone.toSql());
    ...
    expect(
      'select * from "users" on "email" = ? order by "foo" desc, "baz" desc',
    ).toBe(clonedJoin.toSql());
  ...
  it('Works Clone Without', () => {
    ...
    expect('select * from "users" where "email" = ? order by "email" asc').toBe(
      builder.toSql(),
    );
    ...
    expect('select * from "users" on "email" = ? order by "email" asc').toBe(
      join.toSql(),
    );
  ...
  it('Works Clone Without Bindings', () => {
    ...
    expect(['foo']).toEqual(builder.getBindings());
    expect('select * from "users" order by "email" asc').toBe(
      clonedJoin.toSql(),
    );
