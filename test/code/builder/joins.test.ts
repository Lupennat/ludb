...
describe('QueryBuilder Joins', () => {
  it('Works Join Aliases With Prefix', () => {
    ...
    builder
      .select('*')
      .from('services')
      .join('alternative.translations AS t', 't.item_id', '=', 'services.id');
    expect(builder.toSql()).toBe(
      'select * from "prefix_services" inner join "alternative"."prefix_translations" as "prefix_t" on "prefix_t"."item_id" = "prefix_services"."id"',
    );
  });
  ...
  it('Works Basic Joins', () => {
    ...
    expect(
      'select * from "users" left join "photos" on "users"."id" = "bar"',
    ).toBe(builder.toSql());
    ...
    expect(
      'select * from "users" right join "photos" on "users"."id" = "bar"',
    ).toBe(builder.toSql());
    expect([]).toEqual(builder.getBindings());
  ...
  it('Works Joins With Subquery Condition', () => {
    ...
    builder
      ...
      .leftJoin('contacts', (join) => {
        join
          .on('users.id', 'contacts.id')
          .whereIn('contact_type_id', function (query) {
            query
              .select('id')
              .from('contact_types')
              .where('category_id', '1')
  ...
  it('Works Left Join Sub', () => {
    ...
    builder
      .from('users')
      .leftJoinSub(getBuilder().from('contacts'), 'sub', (join) => {
    ...
    let expected = 'select * from "users" ';
    expected +=
      'left join (select * from "contacts" where "name" = ?) as "sub1" on "users"."id" = ? ';
    expected +=
      'inner join (select * from "contacts" where "name" = ?) as "sub2" on "users"."id" = "sub2"."user_id"';
    expect(expected).toEqual(builder.toSql());
    expect(['foo', 1, 'bar']).toEqual(builder.getRawBindings().join);
    ...
    expect(() => {
      builder
        .from('users')
        // @ts-expect-error test wrong argument
        .leftJoinSub(['foo'], 'sub', 'users.id', '=', 'sub.id');
    }).toThrow(
      'A subquery must be a query builder instance, a Closure, or a string.',
    );
  });
  ...
  it('Works Right Join Sub', () => {
    ...
    builder
      .from('users')
      .rightJoinSub(getBuilder().from('contacts'), 'sub', (join) => {
        join.on('users.id', '=', 'sub.id');
      });
    expect(
      'select * from "users" right join (select * from "contacts") as "sub" on "users"."id" = "sub"."id"',
    ...
    let expected = 'select * from "users" ';
    expected +=
      'right join (select * from "contacts" where "name" = ?) as "sub1" on "users"."id" = ? ';
    expected +=
      'inner join (select * from "contacts" where "name" = ?) as "sub2" on "users"."id" = "sub2"."user_id"';
    expect(expected).toEqual(builder.toSql());
    expect(['foo', 1, 'bar']).toEqual(builder.getRawBindings().join);
    ...
    expect(() => {
      builder
        .from('users')
        // @ts-expect-error test wrong argument
        .rightJoinSub(['foo'], 'sub', 'users.id', '=', 'sub.id');
    }).toThrow(
      'A subquery must be a query builder instance, a Closure, or a string.',
    );
  });
