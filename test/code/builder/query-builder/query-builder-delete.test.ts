...
describe('QueryBuilder Methods Delete', () => {
  it('Works Delete Method', async () => {
    ...
    let spiedDelete = jest
      .spyOn(builder.getConnection(), 'delete')
      .mockImplementationOnce(async () => 1);
    expect(
      await builder.from('users').where('email', '=', 'foo').delete(),
    ).toBe(1);
    ...
    spiedDelete = jest
      ...
      .mockImplementationOnce(async () => 1);
    expect(
      await builder.from('users').selectRaw('?', ['ignore']).delete(1),
    ).toBe(1);
    expect(spiedDelete).toHaveBeenCalledWith(
      'delete from "users" where "users"."id" = ?',
      [1],
    ...
    expect(spiedDelete).toHaveBeenCalledWith(
      'delete top (1) from [users] where [email] = ?',
      ['foo'],
    );
  });
  ...
  it('Works Truncate Method', async () => {
    ...
    spiedTruncate = jest
      ...
      .mockImplementation(async () => true);
    await builder.from('users').truncate();
    expect(spiedTruncate).toHaveBeenNthCalledWith(
      1,
    ...
    spiedTruncate = jest
      ...
      .mockImplementation(async () => true);
    await builder.from('users').truncate();
    expect(spiedTruncate).toHaveBeenCalledWith(
