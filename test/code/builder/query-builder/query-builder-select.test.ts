...
describe('QueryBuilder Methods Select', () => {
  ...
  it('Works Find Or Returns First Result By ID', async () => {
    ...
    expect(data).toEqual(
      await builder.findOr(1, ['column'], () => 'callback result'),
    );
    expect('callback result').toBe(
      await builder.findOr(1, () => 'callback result'),
    );
  ...
  it('Works Sole Method Returns Only If Sole', async () => {
    ...
    spiedConnection.mockImplementationOnce(
      async (query, bindings, useReadPdo) => {
    ...
    await expect(
      builder.from('users').where('id', '=', 1).sole<string>('foo'),
  ...
  it('Works Pluck Method Gets An Array Or Object Of Column Values', async () => {
    ...
    await expect(
      builder.from('users').where('id', '=', 1).pluck<string>('foo', 'id'),
    ).rejects.toThrow('key value [Buffer] is not stringable');
    ...
    await expect(
      builder.from('users').where('id', '=', 1).pluck<string>('foo', 'id'),
    ).rejects.toThrow('key value [Array] is not stringable');
    ...
    const results5 = await builder
      .from('users')
      .where('id', '=', 1)
      .pluck<string>('foo', new Raw('baz as id'));
    ...
    const results6 = await builder
      .from('users')
      .where('id', '=', 1)
      .pluck<string>('foo', 'table.id');
  ...
  it('Works RawValue Method Returns Single Column', async () => {
    ...
    spiedConnection.mockImplementationOnce(
      async (query, bindings, useReadPdo) => {
        expect(query).toBe(
          'select UPPER("foo") from "users" where "id" = ? limit 1',
        );
        expect(bindings).toEqual([1]);
        expect(useReadPdo).toBeTruthy();
  ...
  it('Works Aggregate Functions', async () => {
    ...
    spiedConnection.mockImplementationOnce(
      async (query, bindings, useReadPdo) => {
        expect(query).toBe(
          'select count(distinct access, "id") as aggregate from "users"',
        );
        expect(bindings).toEqual([]);
        expect(useReadPdo).toBeTruthy();
        return [{ aggregate: 1 }];
    ...
    expect(
      await builder.from('users').count([new Raw('distinct access'), 'id']),
    ).toBe(1);
    expect(spiedConnection).toHaveBeenCalledTimes(1);
  ...
  it('Works Numeric Aggregate', async () => {
    ...
    spiedConnection
      ...
      .mockImplementationOnce(async (query, bindings, useReadPdo) => {
        expect(query).toBe(
          'select least("id", "access") as aggregate from "users"',
        );
        expect(bindings).toEqual([]);
        expect(useReadPdo).toBeTruthy();
    ...
    expect(await builder.from('users').numericAggregate('count')).toBe(
      BigInt('100'),
    );
    expect(await builder.from('users').numericAggregate('count', ['id'])).toBe(
      10,
    );
    expect(
      await builder.from('users').numericAggregate('least', ['id', 'access']),
    ).toBe(10.4467);
    expect(await builder.from('users').numericAggregate('count', ['id'])).toBe(
      1.0553334343434243e21,
    );
    expect(await builder.from('users').numericAggregate('count', ['id'])).toBe(
  ...
  it('Works Aggregate With SubSelect', async () => {
    ...
    ).toBe(builder.getGrammar().getValue(builder.getRegistry().columns![0]));
  ...
  it('Works Lazy Paginates Using Id Throw Error When Column Is Null Or Does Not Exists', async () => {
    ...
    let chunk1: Array<{ [key: string]: number | null }> = [
      { someIdField: 1 },
      { notId: 2 },
    ];
  ...
  it('Works Cursor Can Be Iterate', async () => {
    ...
    builder.from('users');
    jest
      .spyOn(builder.getConnection(), 'cursor')
      .mockImplementationOnce((sql, bindings, useReadPdo) => {
