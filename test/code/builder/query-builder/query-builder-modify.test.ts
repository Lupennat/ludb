...
describe('QueryBuilder Methods Modify', () => {
  it('Works Insert Method', async () => {
    ...
    jest
      .spyOn(builder.getConnection(), 'insert')
  ...
  it('Works Insert Method With Not Primitive Bindings', async () => {
    ...
    jest
      ...
      .mockImplementationOnce(async (query, bindings) => {
        ...
        expect(bindings).toEqual([
          '{"nested":{"value":[0,"null","922323312312312312312"]}}',
        ]);
        return true;
      })
      .mockImplementationOnce(async (query, bindings) => {
    ...
    expect(
      await builder.from('users').insert({
        json_col: {
          nested: { value: [0, 'null', BigInt('922323312312312312312')] },
        },
      }),
    ).toBeTruthy();
    expect(
      await builder
        .from('users')
        .insert({ json_col: [undefined, new Raw('test'), [null, 'invalid']] }),
    ).toBeTruthy();
    expect(await builder.from('users').insert({})).toBeTruthy();
    expect(await builder.from('users').insert([])).toBeTruthy();
  });
  ...
  it('Works Insert Using Method', async () => {
    ...
    jest
      .spyOn(builder.getConnection(), 'affectingStatement')
      .mockImplementationOnce(async (query, bindings) => {
        expect(query).toBe(
  ...
  it('Works Insert Using With Expression', async () => {
    ...
    expect(
      await builder
        ...
        .withExpression(
          'u',
          getBuilder().from('users').select('id').where('id', '>', 1),
        )
  ...
  it('Works Insert Using With Recursion Limit', async () => {
    ...
    jest
      ...
      .mockImplementationOnce(async (query, bindings) => {
        expect(query).toBe(
          'insert into "posts" ("id") select "id" from "users" option (maxrecursion 100)',
        );
        expect(bindings).toEqual([]);
        return 1;
      });
    ...
    expect(
      await builder
        .from('posts')
        .recursionLimit(100)
        .insertUsing(['id'], getBuilder().from('users').select('id')),
    ).toBe(1);
    ...
    jest
      ...
      .mockImplementationOnce(async (query, bindings) => {
        expect(query).toBe(
          'insert into `posts` (`id`) option (maxrecursion 100) select `id` from `users`',
        );
        expect(bindings).toEqual([]);
        return 1;
      });
  ...
  it('Works Mysql Insert Or Ignore Method', async () => {
    ...
    expect(await builder.from('users').insertOrIgnore({ email: 'foo' })).toBe(
      1,
    );
  });
  ...
  it('Works Postgres Insert Or Ignore Method', async () => {
    ...
    expect(
      await builder
        .from('users')
        .insertOrIgnore([{ email: 'foo' }, { email: 'baz' }]),
    ).toBe(2);
    await expect(
      builder
        .from('users')
        .insertOrIgnore([{ email: 'foo' }, { email: 'baz', name: 'test' }]),
    ).rejects.toThrow('Missing columns [name], please add to each rows.');
    await expect(
      builder
        .from('users')
        .insertOrIgnore([
          { email: 'foo', name: 'test', role: 'test' },
          { email: 'baz' },
        ]),
  ...
  it('Works Insert Get Id Method', async () => {
    let builder = getBuilder();
    jest
      .spyOn(builder.getConnection(), 'insertGetId')
      .mockImplementationOnce(async (query, bindings, sequence) => {
        expect(query).toBe('insert into "users" ("email") values (?)');
        expect(bindings).toEqual(['foo']);
    ...
    expect(
      await builder.from('users').insertGetId({ email: 'foo' }, 'id'),
    ).toBe(1);
    ...
    jest
      ...
      .mockImplementationOnce(async (query, bindings, sequence) => {
        expect(query).toBe(
          'insert into "users" ("email") values (?) returning "idCol"',
        );
        expect(bindings).toEqual(['foo']);
    ...
    expect(
      await builder.from('users').insertGetId({ email: 'foo' }, 'idCol'),
    ).toBe(1);
  });
  ...
  it('Works Insert Method Respects Raw Bindings', async () => {
    ...
    jest
      ...
      .mockImplementationOnce(async (query, bindings) => {
        expect(query).toBe(
          'insert into "users" ("email") values (CURRENT TIMESTAMP)',
        );
        expect(bindings).toEqual([]);
        return true;
      });
  ...
  it('Works Update Method', async () => {
    let builder = getBuilder();
    jest
      .spyOn(builder.getConnection(), 'update')
      .mockImplementationOnce(async (query, bindings) => {
        expect(query).toBe(
  ...
  it('Works Update Method With Expressions', async () => {
    ...
    expect(
      await builder
        ...
        .update({
          views: new Raw('(select count(*) from u)'),
          update_at: '2024-01-03',
        }),
    ).toBe(1);
    ...
    expect(
      await builder
        ...
        .withExpression(
          'u',
          getMysqlBuilder().from('users').where('id', '>', 1),
        )
        .update({
          views: new Raw('(select count(*) from u)'),
          update_at: '2024-01-03',
    ...
    expect(
      await builder
        ...
        .withExpression(
          'u',
          getPostgresBuilder().from('users').where('id', '>', 1),
        )
        .update({
          views: new Raw('(select count(*) from u)'),
          update_at: '2024-01-03',
    ...
    expect(
      await builder
        ...
        .withExpression(
          'u',
          getSqliteBuilder().from('users').where('id', '>', 1),
        )
        .update({
          views: new Raw('(select count(*) from u)'),
          update_at: '2024-01-03',
    ...
    expect(
      await builder
        ...
        .withExpression(
          'u',
          getSqlserverBuilder().from('users').where('id', '>', 1),
        )
        .update({
          views: new Raw('(select count(*) from u)'),
          update_at: '2024-01-03',
  ...
  it('Works Upsert Method', async () => {
    ...
    builder = getMysqlBuilder();
    jest
      .spyOn(builder.getConnection(), 'getConfig')
    ...
    expect(
      await builder
        .from('users')
        .upsert({ email: 'foo', name: 'bar' }, 'email', [
          'name',
          { role: 'fake' },
        ]),
    ).toBe(2);
  ...
  it('Works Upsert Without Update Columns Call Insert', async () => {
    const builder = getMysqlBuilder();
    const spiedInsert = jest
      .spyOn(builder, 'insert')
    ...
    expect(
      await builder.upsert({ name: 'bar2', email: 'foo2' }, 'email', []),
    ).toBe(1);
  ...
  it('Works Update Method Without Joins On Postgres', async () => {
    ...
    jest
      ...
      .mockImplementationOnce(async (query, bindings) => {
        expect(query).toBe(
          'update "users"."users" set "email" = ?, "name" = ? where "id" = ?',
        );
        expect(bindings).toEqual(['foo', 'bar', 1]);
        return 1;
      });
  ...
  it('Works Update From Method With Joins On Postgres', async () => {
    ...
    jest
      ...
      .mockImplementationOnce(async (query, bindings) => {
        expect(query).toBe(
          'update "users" set "email" = ?, "name" = ? where "name" = ?',
        );
    ...
    expect(
      await builder
        ...
        .where('name', 'baz')
        .updateFrom({
          'options->language': ['english', 'italian'],
          'options->size': new Raw("'full'"),
        }),
    ).toBe(1);
  });
  ...
  it('Works Update Method Respects Raw', async () => {
    ...
    jest
      ...
      .mockImplementationOnce(async (query, bindings) => {
        expect(query).toBe(
          'update "users" set "email" = foo, "name" = ? where "id" = ?',
        );
  ...
  it('Works Update Or Insert Method', async () => {
    ...
    expect(
      await builder.updateOrInsert({ email: 'foo' }, { name: 'bar' }),
    ).toBeTruthy();
  ...
  it('Works Increment', async () => {
    ...
    expect(spiedEach).toHaveBeenLastCalledWith(
      { votes: '3' },
      { name: 'Claudio' },
    );
  });
  ...
  it('Works Increment Throw Error When Not Numeric', async () => {
    const builder = getBuilder();
    await expect(builder.increment('votes', 'ab')).rejects.toThrow(
  ...
  it('Works Decrement Throw Error When Not Numeric', async () => {
    const builder = getBuilder();
    await expect(builder.decrement('votes', 'ab')).rejects.toThrow(
