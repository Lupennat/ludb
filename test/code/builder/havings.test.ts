...
describe('QueryBuilder Havings', () => {
  it('Works Having Aggregate', async () => {
    ...
    const spyConnection = jest
      .spyOn(builder.getConnection(), 'select')
      .mockImplementationOnce(async () => []);
    const spyConnectionDatabase = jest.spyOn(
  ...
  it('Works Havings', () => {
    ...
    expect('select * from "users" having "email" = ? or "email" = ?').toBe(
      builder.toSql(),
    );

    builder = getBuilder();
    builder.select('*').from('users').havingNot('email', '>', 1);
    expect('select * from "users" having not "email" > ?').toBe(
      builder.toSql(),
    );
    ...
    expect(
      'select * from "users" having not "email" = ? or not "email" = ?',
    ).toBe(builder.toSql());

    builder = getBuilder();
    builder.select('*').from('users').groupBy('email').having('email', '>', 1);
    expect('select * from "users" group by "email" having "email" > ?').toBe(
      builder.toSql(),
    );
  ...
  it('Works Nested Havings', () => {
    ...
    expect(
      'select * from "users" having "email" = ? or ("name" = ? and "age" = ?)',
    ).toBe(builder.toSql());
    ...
    expect(
      'select * from "users" having not "email" = ? or (not "name" = ? and not "age" = ?)',
    ).toBe(builder.toSql());
    expect(['foo', 'bar', 25]).toEqual(builder.getBindings());
  });
  ...
  it('Works Nested Having Bindings', () => {
    ...
    expect('select * having "email" = ? and ("name" = ?)').toBe(
      builder.toSql(),
    );
    ...
    expect('select * having not "email" = ? and (not "name" = ?)').toBe(
      builder.toSql(),
    );
    expect(['foo', 'bar']).toEqual(builder.getBindings());
  });
  ...
  it('Works Having Betweens', () => {
    ...
    expect('select * from "users" having "id" between ? and ?').toBe(
      builder.toSql(),
    );
    ...
    expect(
      'select * from "users" having "id" between ? and ? or "id" between ? and ?',
    ).toBe(builder.toSql());
    ...
    expect('select * from "users" having "id" not between ? and ?').toBe(
      builder.toSql(),
    );
    expect([1, 2]).toEqual(builder.getBindings());
    ...
    expect(
      'select * from "users" having "id" not between ? and ? or "id" not between ? and ?',
    ).toBe(builder.toSql());
    expect([1, 2, 1, 2]).toEqual(builder.getBindings());
  });
  ...
  it('Works Having Null', () => {
    ...
    expect('select * from "users" having "email" is null').toBe(
      builder.toSql(),
    );
    ...
    expect(
      'select * from "users" having "email" is null and "phone" is null',
    ).toBe(builder.toSql());
    ...
    builder
      ...
      .orHavingNull('phone');
    expect(
      'select * from "users" having "email" is null or "phone" is null',
    ).toBe(builder.toSql());
    ...
    expect(
      'select "email" as "foo_email" from "users" having "foo_email" is null',
    ).toBe(builder.toSql());
  ...
  it('Works Having Not Null', () => {
    ...
    expect('select * from "users" having "email" is not null').toBe(
      builder.toSql(),
    );
    ...
    builder
      ...
      .havingNotNull('phone');
    expect(
      'select * from "users" having "email" is not null and "phone" is not null',
    ).toBe(builder.toSql());
    ...
    expect(
      'select * from "users" having "email" is not null or "phone" is not null',
    ).toBe(builder.toSql());
    ...
    builder
      .select('email as foo_email')
      .from('users')
      .havingNotNull('foo_email');
    expect(
      'select "email" as "foo_email" from "users" having "foo_email" is not null',
    ).toBe(builder.toSql());
  ...
  it('Works Having Nested', () => {
    ...
    builder
      ...
      .havingNested((query) => {
        query
          .having('created_at', '>=', '22:00')
          .orHaving('created_at', '<=', '10:00');
      })
      .havingNested(
        (query) => {
          query
  ...
  it('Works Add Nested Having Query', () => {
    ...
    builder
      ...
      .addNestedHavingQuery(
        getBuilder()
          .having('created_at', '>=', '23:00')
          .orHaving('created_at', '<=', '10:00'),
      );
    expect(builder.toSql()).toBe(
      'select * from "users" having ("created_at" >= ? or "created_at" <= ?)',
    );
  ...
  it('Works Having Followed By Select Get', async () => {
    ...
    expect(spiedConnection).toHaveBeenCalledWith(
      executedQuery,
      ['popular', 3],
      true,
    );
  ...
  it('Works Having Expression', () => {
    ...
    expect(builder.toSql()).toBe(
      'select * from "users" having not user_foo < user_bar',
    );
    expect(builder.getBindings()).toEqual([]);
  });
  ...
  it('Works Raw Havings', () => {
    ...
    expect('select * from "users" having user_foo < user_bar').toBe(
      builder.toSql(),
    );
    ...
    builder
      ...
      .from('users')
      .having('baz', '=', 1)
      .orHavingRaw(new Raw('user_foo < user_bar'));
    expect(
      'select * from "users" having "baz" = ? or user_foo < user_bar',
    ).toBe(builder.toSql());
