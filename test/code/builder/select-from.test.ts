...
describe('QueryBuilder Select-From', () => {
  ...
  it('Works Basic Select Distinct On Columns', () => {
    ...
    builder.distinct('foo').select('foo', 'bar').from('users');
    expect(builder.toSql()).toBe(
      'select distinct on ("foo") "foo", "bar" from "users"',
    );
  });
  ...
  it('Works Mysql Lock', () => {
    ...
    expect('select * from "foo" where "bar" = ? lock in share mode').toBe(
      builder.toSql(),
    );
    expect(['baz']).toEqual(builder.getBindings());
    ...
    builder = getMysqlBuilder();
    builder.select('*').from('foo').where('bar', '=', 'baz').lock();
    expect('select * from `foo` where `bar` = ? for update').toBe(
      builder.toSql(),
    );
    expect(['baz']).toEqual(builder.getBindings());
  ...
  it('Works Postgres Lock', () => {
    ...
    builder = getPostgresBuilder();
    builder.select('*').from('foo').where('bar', '=', 'baz').lock(false);
    expect('select * from "foo" where "bar" = ? for share').toBe(
      builder.toSql(),
    );
    expect(['baz']).toEqual(builder.getBindings());
    ...
    builder
      ...
      .where('bar', '=', 'baz')
      .lock('for key share');
    expect('select * from "foo" where "bar" = ? for key share').toBe(
      builder.toSql(),
    );
    expect(['baz']).toEqual(builder.getBindings());
  });
  ...
  it('Works Sqlserver Lock', () => {
    ...
    builder.select('*').from('foo').where('bar', '=', 'baz').lock(false);
    expect('select * from [foo] with(rowlock,holdlock) where [bar] = ?').toBe(
      builder.toSql(),
    );
    expect(['baz']).toEqual(builder.getBindings());
    ...
    builder
      ...
      .where('bar', '=', 'baz')
      .lock('with(holdlock)');
    expect('select * from [foo] with(holdlock) where [bar] = ?').toBe(
      builder.toSql(),
    );
    expect(['baz']).toEqual(builder.getBindings());
  });
  ...
  it('Works SelectWithLockUsesWritePdo', async () => {
    ...
    jest
      .spyOn(builder.getConnection(), 'select')
      .mockImplementationOnce(async (_a, _b, useReadPdo) => {
  ...
  it('Works From', () => {
    ...
    expect('select * from (select * from "users") as "u"').toBe(
      builder.toSql(),
    );
  });
  ...
  it('Works From Sub', () => {
    ...
    builder
      .fromSub((query) => {
        query
          .select(new Raw('max(last_seen_at) as last_seen_at'))
          .from('user_sessions')
          .where('foo', '=', '1');
      }, 'sessions')
      .where('bar', '<', '10');
  ...
  it('Works From Raw', () => {
    ...
    builder.fromRaw(
      new Raw(
        '(select max(last_seen_at) as last_seen_at from "user_sessions") as "sessions"',
      ),
    );
    expect(
      'select * from (select max(last_seen_at) as last_seen_at from "user_sessions") as "sessions"',
  ...
  it('Works From Raw On Sql Server', () => {
    ...
    expect('select * from dbo.[SomeNameWithRoundBrackets (test)]').toBe(
      builder.toSql(),
    );
  });
  ...
  it('Works From Raw With Where On The Main Query', () => {
    ...
    builder
      .fromRaw(
        new Raw(
          '(select max(last_seen_at) as last_seen_at from "sessions") as "last_seen_at"',
        ),
      )
  ...
  it('Works Use Index Mysql', () => {
    ...
    expect('select `foo` from `users` use index (test_index)').toBe(
      builder.toSql(),
    );
  });
  ...
  it('Works Force Index Mysql', () => {
    ...
    expect('select `foo` from `users` force index (test_index)').toBe(
      builder.toSql(),
    );
  });
  ...
  it('Works Ignore Index Mysql', () => {
    ...
    expect('select `foo` from `users` ignore index (test_index)').toBe(
      builder.toSql(),
    );
  });
  ...
  it('Works Force Index Sqlite', () => {
    const builder = getSqliteBuilder();
    builder.select('foo').from('users').forceIndex('test_index');
    expect('select "foo" from "users" indexed by test_index').toBe(
      builder.toSql(),
    );
  });
  ...
  it('Works Force Index Sqlserver', () => {
    const builder = getSqlserverBuilder();
    builder.select('foo').from('users').forceIndex('test_index');
    expect('select [foo] from [users] with (index(test_index))').toBe(
      builder.toSql(),
    );
  });
