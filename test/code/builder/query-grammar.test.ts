...
import { stringifyReplacer } from '../../../src/utils';
import {
  getBuilder,
  getMysqlBuilder,
  getPostgresBuilder,
...
describe('QueryBuilder Select-From', () => {
  ...
  it('Works Mysql Can Combine Json On Update', async () => {
    ...
    expect(spiedUpdate).toHaveBeenCalledWith(
      ...
      [
        JSON.stringify(
          { enabled: false, tags: ['white', 'large'] },
  ...
  it('Works Mysql Wrapping Json With Boolean', () => {
    ...
    expect(
      'select * from `users` where not json_extract(`items`, \'$."available"\') = true',
    ).toBe(builder.toSql());
    ...
    builder
      .select('*')
      .from('users')
      .where(new Raw("items->'$.available'"), '=', true);
    expect("select * from `users` where items->'$.available' = true").toBe(
      builder.toSql(),
    );
  });
  ...
  it('Works Aggrate', () => {
    ...
    expect('select count(distinct "id") as aggregate from "users"').toBe(
      builder.toSql(),
    );
    builder = getBuilder();
    builder
      ...
      .from('users')
      .distinct('id', 'name')
      .setAggregate('count', ['id']);
    expect(
      'select count(distinct "id", "name") as aggregate from "users"',
    ).toBe(builder.toSql());
  });
  ...
  it('Works Mysql Wrapping Json', () => {
    ...
    expect('select * from `users` where items->\'$."price"\' = 1').toBe(
      builder.toSql(),
    );
  ...
  it('Works Sqlserver Columns', () => {
    let builder = getSqlserverBuilder();
    expect(
      builder.from('users').select('name').setAggregate('count', ['*']).toSql(),
    ).toBe('select count(*) as aggregate from [users]');

    builder = getSqlserverBuilder();
    expect(builder.from('users').select('name').distinct().toSql()).toBe(
  ...
  it('Works Sqlserver Save Points', () => {
    const builder = getSqlserverBuilder();
    expect(builder.getGrammar().compileSavepoint('trans1')).toBe(
      'SAVE TRANSACTION trans1',
    );
    expect(builder.getGrammar().compileSavepointRollBack('trans1')).toBe(
  ...
  it('Works Mysql Sounds Like Operator', () => {
    ...
    expect('select * from `users` where `name` sounds like ?').toBe(
      builder.toSql(),
    );
  ...
  it('Works Bitwise Operators', () => {
    ...
    builder = getPostgresBuilder();
    builder.select('*').from('users').where('bar', '#', 1);
    expect('select * from "users" where ("bar" # ?)::bool').toBe(
      builder.toSql(),
    );
    ...
    expect('select * from "users" where ("range" >> ?)::bool').toBe(
      builder.toSql(),
    );

    builder = getSqlserverBuilder();
    builder.select('*').from('users').where('bar', '&', 1);
    expect('select * from [users] where ([bar] & ?) != 0').toBe(
      builder.toSql(),
    );
    ...
    builder = getPostgresBuilder();
    builder.select('*').from('users').having('bar', '#', 1);
    expect('select * from "users" having ("bar" # ?)::bool').toBe(
      builder.toSql(),
    );
    ...
    expect('select * from "users" having ("range" >> ?)::bool').toBe(
      builder.toSql(),
    );

    builder = getSqlserverBuilder();
    builder.select('*').from('users').having('bar', '&', 1);
    expect('select * from [users] having ([bar] & ?) != 0').toBe(
      builder.toSql(),
    );
  });
