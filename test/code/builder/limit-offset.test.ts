...
describe('QueryBuilder Joins', () => {
  ...
  it('Works For Page Before Id', () => {
    ...
    expect(
      'select * from "users" where "id" < ? order by "id" desc limit 15',
    ).toBe(builder.toSql());
  ...
  it('Works For Page After Id', () => {
    ...
    builder
      .select('*')
      .from('users')
      .orderBy(new Raw('id'), 'desc')
      .forPageAfterId(10, 2, 'id');
    expect(
      'select * from "users" where "id" > ? order by "id" asc limit 10',
    ).toBe(builder.toSql());
    expect([2]).toEqual(builder.getBindings());
    ...
    builder
      ...
      .forPageAfterId(10, 2, 'id');
    expect(
      'select * from "users" where "id" > ? order by name desc, "id" asc limit 10',
    ).toBe(builder.toSql());
    expect([2]).toEqual(builder.getBindings());
  });
});
