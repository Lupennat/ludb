import GrammarBuilderI from '../../../src/types/query/grammar-builder';
import {
  getBuilder,
  getMysqlBuilder,
  getPostgresBuilder,
...
describe('QueryBuilder Cte', () => {
  ...
  it('Works With Recursive Expressions', async () => {
    ...
    let builder = getBuilder()
      .from('numbers')
      .withRecursiveExpression('numbers', query);
  ...
  it('Works Recursion Expression And Cycle Detection', () => {
    ...
    expect(
      ...
      getBuilder()
        .withRecursiveExpressionAndCycleDetection('numbers', query, ['modulo'])
        .toSql(),
    );
  ...
  it('Works Materialized Expression', () => {
    expect(
      'with "u" as materialized (select * from "users") select "u"."id" from "u"',
    ).toBe(
      getBuilder()
        .select('u.id')
        .from('u')
        .withMaterializedExpression('u', getBuilder().from('users'))
        .toSql(),
    );
  ...
  it('Works Non Materialized Expression', () => {
    expect(
      'with "u" as not materialized (select * from "users") select "u"."id" from "u"',
    ).toBe(
      getBuilder()
        .select('u.id')
        .from('u')
        .withNonMaterializedExpression('u', getBuilder().from('users'))
        .toSql(),
    );
  ...
  it('Works Recursion Limit', () => {
    ...
    expect('select * from "users"').toBe(
      getBuilder().from('users').recursionLimit().toSql(),
    );
    expect('select * from `users` option (maxrecursion 100)').toBe(
