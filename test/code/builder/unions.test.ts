...
describe('QueryBuilder Unions', () => {
  ...
  it('Works Unions All', () => {
    ...
    builder.select('*').from('users').where('id', '=', 1);
    builder.unionAll(
      getBuilder().select('*').from('users').where('id', '=', 2),
    );
    expect(
    ...
    builder.select('*').from('users').where('id', '=', 1);
    builder.unionAll(
      getPostgresBuilder().select('*').from('users').where('id', '=', 2),
    );
    expect(
      '(select * from "users" where "id" = ?) union all (select * from "users" where "id" = ?)',
    ).toBe(builder.toSql());
  ...
  it('Works Multiple Union Alls', () => {
    ...
    builder.unionAll(
      getBuilder().select('*').from('users').where('id', '=', 3),
    );
    expect(
  ...
  it('Works Union Aggregate', async () => {
    ...
    await builder
      .from('posts')
      .union(getPostgresBuilder().from('videos'))
      .count();
    expect(spyConnection).toHaveBeenCalledWith(expected, [], true);
    ...
    await builder
      .from('posts')
      .union(getSqliteBuilder().from('videos'))
      .count();
    expect(spyConnection).toHaveBeenCalledWith(expected, [], true);
    ...
    await builder
      .from('posts')
      .union(getSqlserverBuilder().from('videos'))
      .count();
    expect(spyConnection).toHaveBeenCalledWith(expected, [], true);
  });
});
