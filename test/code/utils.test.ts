...
describe('Utils', () => {
  ...
  it('Works Stringify Replacer', () => {
    expect(
      JSON.stringify({ a: BigInt('10') }, stringifyReplacer(new Grammar())),
    ).toBe('{"a":"10"}');
    expect(
      JSON.stringify({ a: raw('Test') }, stringifyReplacer(new Grammar())),
    ).toBe('{"a":"Test"}');
    expect(
      JSON.stringify(
        { a: raw(BigInt('100')) },
        stringifyReplacer(new Grammar()),
      ),
    ).toBe('{"a":"100"}');
  });
  ...
  it('Works Get Messages From Error', () => {
    ...
    expect(getMessagesFromError(secondError)).toEqual([
      'second error',
      'first error',
    ]);
    ...
    expect(getMessagesFromError(thirdError)).toEqual([
      'third error',
      'second error',
      'first error',
    ]);
  ...
  it('Works Caused By Lost Connection', () => {
    ...
    expect(
      causedByLostConnection(new Error('noway server has gone away otherwise')),
    ).toBeTruthy();
  ...
  it('Works Caused By Concurrency Error', () => {
    ...
    expect(
      causedByConcurrencyError(new Error('here deadlock detected go')),
    ).toBeTruthy();
  ...
  it('Works Before Last', () => {
    ...
    expect(
      beforeLast('testBeforeLastSearchTerminateHereSearch', 'Search'),
    ).toBe('testBeforeLastSearchTerminateHere');
  });
  ...
  it('Works After Last', () => {
    ...
    expect(
      afterLast('afterLastSearchShouldBeSkippedSearchNotThis', 'Search'),
    ).toBe('NotThis');
  });
