...
import { getConnection, MockedSchemaBuilder } from '../fixtures/mocked';
...
describe('Schema QueryBuilder Test', () => {
  ...
  it('Works Drop Tables', async () => {
    const session = getConnection().sessionSchema();
    const builder = new QueryBuilder(session);
    await expect(builder.dropTables()).rejects.toThrow(
  ...
  it('Works Drop Types', async () => {
    const session = getConnection().sessionSchema();
    const builder = new QueryBuilder(session);
    await expect(builder.dropTypes()).rejects.toThrow(
  ...
  it('Works Drop Views', async () => {
    const session = getConnection().sessionSchema();
    const builder = new QueryBuilder(session);
    await expect(builder.dropViews()).rejects.toThrow(
  ...
  it('Works Drop View', async () => {
    const session = getConnection().sessionSchema();
    const builder = new QueryBuilder(session);
    await expect(builder.dropView('foo')).rejects.toThrow(
      'This database driver does not support dropping views.',
    );
  });
  ...
  it('Works Drop Type', async () => {
    const session = getConnection().sessionSchema();
    const builder = new QueryBuilder(session);
    await expect(builder.dropType('foo')).rejects.toThrow(
      'This database driver does not support dropping types.',
    );
  });
  ...
  it('Works Get Tables', async () => {
    const session = getConnection().sessionSchema();
    const builder = new QueryBuilder(session);
    await expect(builder.getTables()).rejects.toThrow(
      'This database driver does not support retrieval of tables.',
    );
  });
  ...
  it('Works Get Views', async () => {
    const session = getConnection().sessionSchema();
    const builder = new QueryBuilder(session);
    await expect(builder.getViews()).rejects.toThrow(
      'This database driver does not support retrieval of views.',
    );
  });
  ...
  it('Works Get Types', async () => {
    const session = getConnection().sessionSchema();
    const builder = new QueryBuilder(session);
    await expect(builder.getTypes()).rejects.toThrow(
  ...
  it('Works Get Column Type', async () => {
    ...
    expect(await builder.getColumnType('table', 'column', true)).toBe(
      'varchar(255)',
    );
  ...
  it('Works Drop', async () => {
    const session = getConnection().sessionSchema();
    const builder = new MockedSchemaBuilder(session);
    await expect(builder.drop('users')).rejects.toThrow(
      'This database driver does not support drop table.',
    );
  });
  ...
  it('Works Without Foreign Key Constraints', async () => {
    ...
    const called: string[] = [];
    jest
      .spyOn(builder, 'disableForeignKeyConstraints')
      .mockImplementationOnce(async () => {
        called.push('disabled');
        return true;
      });
    jest
      .spyOn(builder, 'enableForeignKeyConstraints')
      .mockImplementationOnce(async () => {
        called.push('enabled');
        return true;
      });
