...
import SqlserverSchemaGrammar from '../../../../src//schema/grammars/sqlserver-grammar';
import {
...
describe('Sqlserver Schema Grammar', () => {
  it('Works Basic Create Table', () => {
    const connection = getSqlserverConnection().sessionSchema();
  ...
  it('Works Drop Spatial Index', () => {
    ...
    expect(1).toBe(statements.length);
    expect('drop index [geo_coordinates_spatialindex] on [geo]').toBe(
      statements[0],
    );
  });
  ...
  it('Works Drop Constrained Foreign Id', () => {
    ...
    expect('alter table [users] drop constraint [users_foo_foreign]').toBe(
      statements[0],
    );
    expect(
      "DECLARE @sql NVARCHAR(MAX) = '';SELECT @sql += 'ALTER TABLE [dbo].[users] DROP CONSTRAINT ' + OBJECT_NAME([default_object_id]) + ';' FROM sys.columns WHERE [object_id] = OBJECT_ID('[dbo].[users]') AND [name] in ('foo') AND [default_object_id] <> 0;EXEC(@sql);alter table [users] drop column [foo]",
  ...
  it('Works Adding Primary Key', () => {
    ...
    expect(1).toBe(statements.length);
    expect('alter table [users] add constraint [bar] primary key ([foo])').toBe(
      statements[0],
    );
  });
  ...
  it('Works Adding Foreign Id Specifying Index Name In Constraint', () => {
    ...
    expect(2).toBe(statements.length);
    expect('alter table [users] add [company_id] bigint not null').toBe(
      statements[0],
    );
    expect(
  ...
  it('Works Adding Char Without Length Limit', () => {
    ...
    expect(1).toBe(statements.length);
    expect('alter table [users] add [foo] nchar(255) not null').toBe(
      statements[0],
    );
    ...
    try {
      expect(1).toBe(statements.length);
      expect('alter table [users] add [foo] nchar not null').toBe(
        statements[0],
      );
    } finally {
  ...
  it('Works Adding String', () => {
    ...
    expect(1).toBe(statements.length);
    expect('alter table [users] add [foo] nvarchar(255) not null').toBe(
      statements[0],
    );
    ...
    expect(1).toBe(statements.length);
    expect('alter table [users] add [foo] nvarchar(100) not null').toBe(
      statements[0],
    );
  ...
  it('Works Adding String Without Length Limit', () => {
    ...
    try {
      expect(1).toBe(statements.length);
      expect('alter table [users] add [foo] nvarchar not null').toBe(
        statements[0],
      );
    } finally {
      QueryBuilder.withDefaultStringLength(255);
  ...
  it('works Column Modifying', () => {
    ...
    blueprint = getSqlserverBlueprint('users', (table) => {
      table
        .computed('discounted_virtual', 'price - 5')
        .persisted(false)
        .change();
    });
  ...
  it('Works Adding Text', () => {
    ...
    expect(1).toBe(statements.length);
    expect('alter table [users] add [foo] nvarchar(max) not null').toBe(
      statements[0],
    );
  });
  ...
  it('Works Adding Year', () => {
    ...
    const statements = blueprint.toSql(connection);
    expect(1).toBe(statements.length);
    expect('alter table [users] add [birth_year] int not null').toBe(
      statements[0],
    );
  });
  ...
  it('Works Adding Date Time', () => {
    ...
    const statements = blueprint.toSql(connection);
    expect(1).toBe(statements.length);
    expect('alter table [users] add [created_at] datetime2 not null').toBe(
      statements[0],
    );
  });
  ...
  it('Works Adding Date Time With Default Current', () => {
    ...
    expect(1).toBe(statements.length);
    expect(
      'alter table [users] add [foo] datetime2 not null default CURRENT_TIMESTAMP',
    ).toBe(statements[0]);
  });
  ...
  it('Works Adding Date Time With Precision', () => {
    ...
    const statements = blueprint.toSql(connection);
    expect(1).toBe(statements.length);
    expect('alter table [users] add [created_at] datetime2(0) not null').toBe(
      statements[0],
    );
  });
  ...
  it('Works Adding Date Time Tz', () => {
    ...
    const statements = blueprint.toSql(connection);
    expect(1).toBe(statements.length);
    expect('alter table [users] add [foo] datetimeoffset not null').toBe(
      statements[0],
    );
  });
  ...
  it('Works Adding Date Time Tz With Default Current', () => {
    ...
    expect(1).toBe(statements.length);
    expect(
      'alter table [users] add [foo] datetimeoffset not null default CURRENT_TIMESTAMP',
    ).toBe(statements[0]);
  });
  ...
  it('Works Adding Date Time Tz With Precision', () => {
    ...
    const statements = blueprint.toSql(connection);
    expect(1).toBe(statements.length);
    expect('alter table [users] add [foo] datetimeoffset(0) not null').toBe(
      statements[0],
    );
  });
  ...
  it('Works Adding Timestamp Tz', () => {
    ...
    const statements = blueprint.toSql(connection);
    expect(1).toBe(statements.length);
    expect('alter table [users] add [created_at] datetimeoffset not null').toBe(
      statements[0],
    );
  });
  ...
  it('Works Adding Timestamp Tz With Precision', () => {
    ...
    expect(1).toBe(statements.length);
    expect(
      'alter table [users] add [created_at] datetimeoffset(0) not null',
    ).toBe(statements[0]);
  });
  ...
  it('Works Adding Timestamps', () => {
    ...
    expect(1).toBe(statements.length);
    expect(
      'alter table [users] add [created_at] datetime2 null, [updated_at] datetime2 null',
    ).toBe(statements[0]);
  });
  ...
  it('Works Adding Binary', () => {
    ...
    expect(1).toBe(statements.length);
    expect('alter table [users] add [foo] varbinary(max) not null').toBe(
      statements[0],
    );
  });
  ...
  it('Works Adding Uuid', () => {
    ...
    expect(1).toBe(statements.length);
    expect('alter table [users] add [foo] uniqueidentifier not null').toBe(
      statements[0],
    );
  });
  ...
  it('Works Adding Uuid Defaults Column Name', () => {
    ...
    expect(1).toBe(statements.length);
    expect('alter table [users] add [uuid] uniqueidentifier not null').toBe(
      statements[0],
    );
  });
  ...
  it('Works Adding Ip Address', () => {
    ...
    expect(1).toBe(statements.length);
    expect('alter table [users] add [foo] nvarchar(45) not null').toBe(
      statements[0],
    );
  });
  ...
  it('Works Adding Ip Address Defaults Column Name', () => {
    ...
    expect(1).toBe(statements.length);
    expect('alter table [users] add [ip_address] nvarchar(45) not null').toBe(
      statements[0],
    );
  });
  ...
  it('Works Adding Mac Address', () => {
    ...
    expect(1).toBe(statements.length);
    expect('alter table [users] add [foo] nvarchar(17) not null').toBe(
      statements[0],
    );
  });
  ...
  it('Works Adding Mac Address Defaults Column Name', () => {
    ...
    expect(1).toBe(statements.length);
    expect('alter table [users] add [mac_address] nvarchar(17) not null').toBe(
      statements[0],
    );
  });
  ...
  it('Works Adding Geometry', () => {
    ...
    expect(1).toBe(statements.length);
    expect('alter table [geo] add [coordinates] geography not null').toBe(
      statements[0],
    );
  });
  ...
  it('Works Quote String', () => {
    expect("N'中文測試'").toBe(
      new SqlserverSchemaGrammar().quoteString('中文測試'),
    );
  });

  it('Works Quote String On Array', () => {
    expect("N'中文', N'測試'").toBe(
      new SqlserverSchemaGrammar().quoteString(['中文', '測試']),
    );
  });

  it('Works Create Database', () => {
    const statement = new SqlserverSchemaGrammar().compileCreateDatabase(
      'my_database_a',
    );
  ...
  it('Works Drop Database If Exists', () => {
    let statement = new SqlserverSchemaGrammar().compileDropDatabaseIfExists(
      'my_database_a',
    );

    expect('drop database if exists [my_database_a]').toBe(statement);

    statement = new SqlserverSchemaGrammar().compileDropDatabaseIfExists(
      'my_database_b',
    );
