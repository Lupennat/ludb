...
describe('Postgres Schema QueryBuilder Test', () => {
  ...
  it('Works Create Type', async () => {
    ...
    jest
      ...
      .mockImplementationOnce(async (sql, bindings) => {
        expect(sql).toBe(
          'create type array_type as (f1 int, f2 text collate "und-x-icu")',
        );
        expect(bindings).toBeUndefined();
        return true;
      })
      ...
      .mockImplementationOnce(async (sql, bindings) => {
        expect(sql).toBe(
          "create type enum as enum ('enum1', 'enum2', 'enum3')",
        );
        expect(bindings).toBeUndefined();
        return true;
      })
    ...
    expect(
      await builder.createType('enum', 'enum', ['enum1', 'enum2', 'enum3']),
    ).toBeTruthy();
  ...
  it('Works Drop Tables', async () => {
    ...
    const stmtSpied = jest
      ...
      .mockImplementationOnce(async (sql, bindings) => {
        expect(sql).toBe(
          'drop table "public"."users","public"."companies" cascade',
        );
        expect(bindings).toBeUndefined();
        return true;
      })
      .mockImplementationOnce(async (sql, bindings) => {
        expect(sql).toBe(
          'drop table "public"."users","public"."spatial_ref_sys" cascade',
        );
        expect(bindings).toBeUndefined();
        return true;
      });
  ...
  it('Works Drop Views', async () => {
    ...
    const stmtSpied = jest
      ...
      .mockImplementationOnce(async (sql, bindings) => {
        expect(sql).toBe(
          'drop view "public"."view_users","public"."view_companies" cascade',
        );
        expect(bindings).toBeUndefined();
        return true;
      })
