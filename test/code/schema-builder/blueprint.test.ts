...
import { Relatable } from '../../../src/types/schema/blueprint';
import {
  getConnection,
  getMysqlBlueprint,
...
describe('Blueprint', () => {
  ...
  it('Works Index Default Names When Prefix Supplied', () => {
    ...
    command = blueprint.spatialIndex('coordinates');
    expect(command.getRegistry().index).toBe(
      'prefix_geo_coordinates_spatialindex',
    );
  ...
  it('Works Default Current Date Time', () => {
    ...
    expect([
      'alter table `users` add `created` datetime(0) not null default CURRENT_TIMESTAMP',
    ...
    expect([
      'alter table "users" add column "created" datetime not null default CURRENT_TIMESTAMP',
    ]).toEqual(blueprint.toSql(connection));
    blueprint = getSqlserverBlueprint('users', (table) => {
      table.dateTime('created').useCurrent();
    });
    expect([
      'alter table [users] add [created] datetime2(0) not null default CURRENT_TIMESTAMP',
    ]).toEqual(blueprint.toSql(connection));
  });
  ...
  it('Works Default Current Timestamp', () => {
    ...
    expect([
      'alter table `users` add `created` timestamp(0) not null default CURRENT_TIMESTAMP',
    ]).toEqual(blueprint.toSql(connection));
    blueprint = getPostgresBlueprint('users', (table) => {
      table.timestamp('created').useCurrent();
    });
  ...
  it('Works Unsigned Decimal Table', () => {
    ...
    expect([
      'alter table "users" add column "money" decimal(10, 2) not null',
    ]).toEqual(blueprint.toSql(connection));
    blueprint = getSqliteBlueprint('users', (table) => {
      table.unsignedDecimal('money', 10, 2);
    });
    expect(['alter table "users" add column "money" numeric not null']).toEqual(
      blueprint.toSql(connection),
    );
    blueprint = getSqlserverBlueprint('users', (table) => {
      table.unsignedDecimal('money', 10, 2);
    });
    expect(['alter table [users] add [money] decimal(10, 2) not null']).toEqual(
      blueprint.toSql(connection),
    );
  });
  ...
  it('Works Remove Column', () => {
    ...
    expect(['alter table `users` add `foo` varchar(255) not null']).toEqual(
      blueprint.toSql(connection),
    );
  });
  ...
  it('Works Rename Column', () => {
    ...
    expect(['alter table `users` rename column `foo` to `bar`']).toEqual(
      blueprint.toSql(connection),
    );
    blueprint = getPostgresBlueprint('users', (table) => {
      table.renameColumn('foo', 'bar');
    ...
    expect(["sp_rename '[users].[foo]', [bar], 'COLUMN'"]).toEqual(
      blueprint.toSql(connection),
    );
  });
  ...
  it('Works Drop Column', () => {
    ...
    expect(['alter table `users` drop `foo`']).toEqual(
      blueprint.toSql(connection),
    );
    blueprint = getPostgresBlueprint('users', (table) => {
      table.dropColumn('foo');
    });
    expect(['alter table "users" drop column "foo"']).toEqual(
      blueprint.toSql(connection),
    );
    blueprint = getSqliteBlueprint('users', (table) => {
      table.dropColumn('foo');
  ...
  it('Works Generate Relationship Column With Incremental Model', () => {
    ...
    expect(['alter table [posts] add [user_id] bigint not null']).toEqual(
      blueprint.toSql(connection),
    );
  });
  ...
  it('Works Generate Relationship Column With Uuid Model', () => {
    ...
    expect(['alter table `posts` add `user_uuid` char(36) not null']).toEqual(
      blueprint.toSql(connection),
    );
    blueprint = getPostgresBlueprint('posts', (table) => {
      table.foreignIdFor(User);
    ...
    expect([
      'alter table "posts" add column "user_uuid" uuid not null',
    ]).toEqual(blueprint.toSql(connection));
    blueprint = getSqliteBlueprint('posts', (table) => {
      table.foreignIdFor(User);
    });
    expect([
      'alter table "posts" add column "user_uuid" varchar not null',
    ]).toEqual(blueprint.toSql(connection));
    blueprint = getSqlserverBlueprint('posts', (table) => {
      table.foreignIdFor(new User());
    });
    expect([
      'alter table [posts] add [user_uuid] uniqueidentifier not null',
    ]).toEqual(blueprint.toSql(connection));
  });
  ...
  it('Works Tiny Text Column', () => {
    ...
    expect(['alter table `posts` add `note` tinytext not null']).toEqual(
      blueprint.toSql(connection),
    );
    blueprint = getPostgresBlueprint('posts', (table) => {
      table.tinyText('note');
    ...
    expect(['alter table "posts" add column "note" text not null']).toEqual(
      blueprint.toSql(connection),
    );
    blueprint = getSqlserverBlueprint('posts', (table) => {
      table.tinyText('note');
    });
    expect(['alter table [posts] add [note] nvarchar(255) not null']).toEqual(
      blueprint.toSql(connection),
    );
  });
  ...
  it('Works Tiny Text Nullable Column', () => {
    ...
    expect(['alter table `posts` add `note` tinytext null']).toEqual(
      blueprint.toSql(connection),
    );
    blueprint = getPostgresBlueprint('posts', (table) => {
      table.tinyText('note').nullable();
    });
    expect(['alter table "posts" add column "note" varchar(255) null']).toEqual(
      blueprint.toSql(connection),
    );
    blueprint = getSqliteBlueprint('posts', (table) => {
      table.tinyText('note').nullable();
    });
    expect(['alter table "posts" add column "note" text']).toEqual(
      blueprint.toSql(connection),
    );
    blueprint = getSqlserverBlueprint('posts', (table) => {
      table.tinyText('note').nullable();
    });
    expect(['alter table [posts] add [note] nvarchar(255) null']).toEqual(
      blueprint.toSql(connection),
    );
  });
