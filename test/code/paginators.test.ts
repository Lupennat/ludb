import Cursor from '../../src/paginations/cursor';
import CursorPaginator from '../../src/paginations/cursor-paginator';
import LengthAwarePaginator from '../../src/paginations/length-aware-paginator';
import Paginator from '../../src/paginations/paginator';
import { Objectable } from '../../src/types/query/grammar-builder';

describe('Paginators', () => {
  it('Works Length Aware Paginator', async () => {
    let paginator = new LengthAwarePaginator(
    ...
    expect(paginator.nextPageUrl()).toBe(
      '/path?name=2&param1=value1&param2=value2&param3=value3#test',
    );
    ...
    expect(paginator.toObject()).toEqual({
      ...
      first_page_url:
        '/path?name=1&param1=value1&param2=value2&param3=value3#test',
      from: 1,
      prev_page_url: null,
      path: '/path',
      per_page: 4,
      ...
      last_page: 3,
      last_page_url:
        '/path?name=3&param1=value1&param2=value2&param3=value3#test',
      total: 10,
    });
    expect(JSON.stringify(paginator)).toEqual(
      JSON.stringify(paginator.toObject()),
    );
    expect(paginator.total()).toBe(10);
    expect(paginator.url(2)).toBe(
      '/path?name=2&param1=value1&param2=value2&param3=value3#test',
    );
  ...
  it('Works Cursor', async () => {
    ...
    expect(['2023-03-25 10:00:00', '422']).toEqual(
      cursor.parameters(['created_at', 'id']),
    );
  ...
  it('Works Cursor Paginator', async () => {
    ...
    expect(paginator.getCursorForItem({ id: null })).toEqual(
      new Cursor({ 'test.id': '' }, true),
    );
    ...
    expect(() => {
      ...
      'Only plain objects are supported when cursor paginating items.',
    );
    ...
    expect(paginator.getCursorForItem(new Test2())).toEqual(
      new Cursor({ 'test.id': '2' }, true),
    );
