import { GLOBAL_CACHE } from '../../src/utils';
...
describe('Cache Manager', () => {
  ...
  it('Works Cache Driver will be shared', () => {
    ...
    expect(GLOBAL_CACHE in cache.getCaches()).toBeTruthy();
    expect('sqlsrv' in cache.getCaches()).toBeTruthy();
    expect(cache.getCaches()[GLOBAL_CACHE]).toBeInstanceOf(MockedCacheDriver);
  ...
  it('Works get Cache Name', () => {
    ...
    expect(cache.getCacheName('sqlite')).toEqual(GLOBAL_CACHE);
  });
  ...
  it('Works get Cache Config', () => {
    ...
    expect(cache.getCacheConfig('sqlite')).toEqual({
      options: { preserve: false },
    });
  });
  ...
  it('Works Get Call Driver Cache Get', async () => {
    ...
    expect(spiedGet).toHaveBeenNthCalledWith(
      1,
      { duration: 60000, key: 'test', time: 10 },
      {},
    );
    expect(spiedGet).toHaveBeenNthCalledWith(
      2,
