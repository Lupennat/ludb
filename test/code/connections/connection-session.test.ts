...
import DeadlockError from '../../../src/errors/deadlock-error';
import QueryExecuted from '../../../src/events/query-executed';
...
import TransactionCommitting from '../../../src/events/transaction-committing';
import TransactionRolledBack from '../../../src/events/transaction-rolledback';
...
describe('Connection Session', () => {
  ...
  it('Works Select Use Read Pdo', async () => {
    ...
    const session = new MockedConnectionSession(connection);
    const spiedPdo = jest
      .spyOn(session, 'getPdoForSelect')
      .mockImplementation(() => pdo);
  ...
  it('Works Select With Pretend Return Empty', async () => {
    const connection = getConnection();
    const session = new MockedConnectionSession(connection);
    const spiedPretending = jest
      .spyOn(session, 'pretending')
  ...
  it('Works Select Return Array Of Results', async () => {
    const connection = getConnection();
    const session = new MockedConnectionSessionWithResults(
      connection,
      [[1], [2], [3]],
      ['test'],
    );
  ...
  it('Works Select Return Array Of Columns', async () => {
    ...
    jest.spyOn(session, 'getPdoForSelect').mockReturnValueOnce(pdo);
    expect(
      await session.selectColumn<number>(0, 'select * from users', []),
    ).toEqual([1, 2, 3]);
    await pdo.disconnect();
  });
  ...
  it('Works Select One', async () => {
    const connection = getConnection();
    let session = new MockedConnectionSessionWithResults(
      connection,
      [[1], [2], [3]],
      ['test'],
    );
    ...
    expect(await session.selectOne('select * from users', [], true)).toEqual({
      test: 1,
    });
  ...
  it('Works Select Resultsets Return Multiple Rowset', async () => {
    ...
    jest.spyOn(session, 'getPdoForSelect').mockReturnValueOnce(pdo);
    expect(
      await session.selectResultSets('CALL a_procedure(?)', ['foo']),
    ).toEqual([
  ...
  it('Works Scalar', async () => {
    ...
    session = new MockedConnectionSessionWithResults(
      connection,
      [[10, 20]],
      ['test', 'test2'],
    );
    jest.spyOn(session, 'getPdoForSelect').mockReturnValueOnce(pdo);
    await expect(session.scalar('select * from users', [])).rejects.toThrow(
  ...
  it('Works Select From Write Connection', async () => {
    ...
    const spiedSelect = jest.spyOn(session, 'select');
    expect(
      await session.selectFromWriteConnection('select * from users', []),
  ...
  it('Works Cursor Query Will Be Prepared', async () => {
    ...
    expect(
      Array.from(
        await session.cursor('select * from users', [null, 'claudio']),
      ),
    ).toEqual([]);
    expect(spiedPrepare).toHaveBeenCalledWith('select * from users');
    expect(spiedBindings).toHaveBeenCalledWith([null, 'claudio']);
  ...
  it('Works Statment Query Will Be Prepared', async () => {
    ...
    const spiedValues = jest.spyOn(session, 'bindValues');
    expect(
      await session.statement('insert into "users" ("email") values (?)', [
        'foo',
      ]),
    ).toBeTruthy();
    expect(spiedPrepare).toHaveBeenCalledWith(
  ...
  it('Works Insert Get Id Will Be Prepared', async () => {
    ...
    const spiedValues = jest.spyOn(session, 'bindValues');
    expect(
      await session.insertGetId('insert into "users" ("email") values (?)', [
        'foo',
      ]),
    ).toBe('idValue');
    expect(spiedPrepare).toHaveBeenCalledWith(
  ...
  it('Works Insert Get Id Return Id', async () => {
    ...
    jest.spyOn(pdo, 'prepare').mockImplementationOnce(async (sql) => {
      const statement = await originalPrepare.call(pdo, sql);
      jest
        .spyOn(statement, 'lastInsertId')
        .mockImplementationOnce(async (sequence) => {
  ...
  it('Works Affecting Statement Query Will Be Prepared', async () => {
    ...
    const spiedValues = jest.spyOn(session, 'bindValues');
    expect(
      await session.affectingStatement(
  ...
  it('Works Update Call Affecting Statement', async () => {
    ...
    const session = new MockedConnectionSession(connection);
    const spiedStatement = jest
      .spyOn(session, 'affectingStatement')
  ...
  it('Works Delete Call Affecting Statement', async () => {
    ...
    const spiedStatement = jest
      ...
      .mockReturnValueOnce(Promise.resolve(0));
    expect(
      await session.delete('delete from "users" where "email" = ?', ['foo']),
    ).toBe(0);
    expect(spiedStatement).toHaveBeenCalledWith(
      'delete from "users" where "email" = ?',
      ['foo'],
  ...
  it('Works Unprepare Call Pdo Exec', async () => {
    ...
    expect(spiedExec).toHaveBeenCalledWith(
      'delete from "users" where "email" = "foo"',
    );
    await pdo.disconnect();
  });
  ...
  it('Works Run Will Be Called Before Executing Pdo', async () => {
    ...
    await session.affectingStatement('delete from "users" where "email" = ?', [
      'foo',
    ]);
    await session.insertGetId('insert into "users" ("email") values (?)', [
      'foo',
  ...
  it('Works Run Throw Query Error', async () => {
    ...
    await expect(
      session.run('delete from "users" where "email" = ?', ['foo'], callback),
    ).rejects.toThrow(
  ...
  it('Works Pretend Return Array of Logged Queries', async () => {
    ...
    expect(
      ...
      {
        bindings: ['foo'],
        query: 'insert into "users" ("email") values (\'foo\')',
      },
  ...
  it('Works Pretend Reset Query Logger', async () => {
    ...
    expect(
      await session.pretend(async (ses) => {
        await ses.delete('delete from "users" where "email" = "foo"', [
          'foo',
          'baz',
        ]);
      }),
      ...
      {
        bindings: ['foo', 'baz'],
        query: 'delete from "users" where "email" = "foo"',
      },
    ]);
  });
  ...
  it('Works Begin Transaction', async () => {
    ...
    const spiedEnsured = jest
      .spyOn(session, 'getEnsuredPdo')
      .mockReturnValueOnce(pdo);
  ...
  it('Works Transaction Call Begin Transaction', async () => {
    const connection = getConnection();
    const session = new MockedConnectionSession(connection);
    const spiedBeginTransaction = jest
      .spyOn(session, 'beginTransaction')
      .mockImplementation();
  ...
  it('Works Transaction Attempts On Commit Error', async () => {
    ...
    jest.spyOn(session, 'getEnsuredPdo').mockReturnValue(pdo);
    await expect(session.transaction(async () => {}, 5)).rejects.toThrow(
      'server has gone away',
    );
    expect(spiedBeginTransaction).toHaveBeenCalledTimes(1);
    expect(spiedCommit).toHaveBeenCalledTimes(1);
  ...
  it('Works Select Use Cache', async () => {
    const connection = getConnection();
    const session = new MockedConnectionSession(connection);
    const spiedManager = jest
      .spyOn(session, 'getCacheManager')
  ...
  it('Works Select From Cache Check Expire', async () => {
    ...
    await session.select('select * from users', [1, 2, 4]);
    await session.select('select * from users', [1, 2, 4]);
    expect(await session.select('select * from users', [1, 2, 4])).toEqual([
      { id: 1 },
      { id: 2 },
    ]);
  ...
  it('Works Select Store Data On Cache', async () => {
    ...
    const spiedStore = jest
      .spyOn(cacheManager, 'store')
      .mockReturnValue(Promise.resolve(undefined));
    const connection = getConnection();
    const session = new MockedConnectionSession(connection);
    const spiedManager = jest
