...
import { bindTo } from '../../../src/bindings/bind.to';
import CacheManager from '../../../src/cache-manager';
import ConnectionSession from '../../../src/connections/connection-session';
...
import MysqlGrammar from '../../../src/query/grammars/mysql-grammar';
import PostgresGrammar from '../../../src/query/grammars/postgres-grammar';
import SqliteGrammar from '../../../src/query/grammars/sqlite-grammar';
import SqlserverGrammar from '../../../src/query/grammars/sqlserver-grammar';
import QueryBuilder from '../../../src/query/query-builder';
import SchemaBuilder from '../../../src/schema/builders/builder';
import MysqlBuilder from '../../../src/schema/builders/mysql-builder';
import PostgresBuilder from '../../../src/schema/builders/postgres-builder';
import SqliteBuilder from '../../../src/schema/builders/sqlite-builder';
import SqlserverBuilder from '../../../src/schema/builders/sqlserver-builder';
...
describe('Connection', () => {
  ...
  it('Works Query Grammar', () => {
    ...
    expect(getPostgresConnection().getQueryGrammar()).toBeInstanceOf(
      PostgresGrammar,
    );
    expect(getSqliteConnection().getQueryGrammar()).toBeInstanceOf(
      SqliteGrammar,
    );
    expect(getSqlserverConnection().getQueryGrammar()).toBeInstanceOf(
      SqlserverGrammar,
    );
  });
  ...
  it('Works Schema QueryBuilder', () => {
    ...
    expect(getMysqlConnection().getSchemaBuilder()).toBeInstanceOf(
      MysqlBuilder,
    );
    expect(getPostgresConnection().getSchemaBuilder()).toBeInstanceOf(
      PostgresBuilder,
    );
    expect(getSqliteConnection().getSchemaBuilder()).toBeInstanceOf(
      SqliteBuilder,
    );
    expect(getSqlserverConnection().getSchemaBuilder()).toBeInstanceOf(
      SqlserverBuilder,
    );
  });
  ...
  it('Works Prepare Bindings', () => {
    const connection = getConnection();
    expect(connection.prepareBindings([null, new Raw('expression')])).toEqual([
      null,
      'expression',
    ]);
    expect(
  ...
  it('Works Get Config', () => {
    ...
    expect(
      connection.getConfig<boolean>('pool.killResource', true),
    ).toBeFalsy();
  ...
  it('Works Query', () => {
    const connection = getConnection();
    const session = new ConnectionSession(connection);
    const spiedSession = jest
      .spyOn(connection, 'session')
  ...
  it('Works Cache', () => {
    ...
    expect(
      connection.cache({ cache: 1000, key: 'test', options: { a: true } }),
    ).toBeInstanceOf(ConnectionSession);
    expect(spiedSession).toHaveBeenCalledTimes(1);
    expect(spiedSession).toHaveBeenCalledWith({
      cache: 1000,
      key: 'test',
      options: { a: true },
    });
  ...
  it('Works Select Resultsets', async () => {
    ...
    jest.spyOn(connection, 'session').mockReturnValue(session);
    const spiedSession = jest
      .spyOn(session, 'selectResultSets')
      .mockImplementation();
  ...
  it('Works Select From Write Connection', async () => {
    ...
    jest.spyOn(connection, 'session').mockReturnValue(session);
    const spiedSession = jest
      .spyOn(session, 'selectFromWriteConnection')
      .mockImplementation();
  ...
  it('Works Insert', async () => {
    ...
    jest.spyOn(connection, 'session').mockReturnValue(session);
    const spiedSession = jest
      .spyOn(session, 'insert')
      .mockImplementation(async () => true);
    await connection.insert('insert into "users" ("email") values (?)', [
      'foo',
    ]);
    expect(spiedSession).toHaveBeenCalledWith(
  ...
  it('Works Insert Get Id', async () => {
    ...
    jest.spyOn(connection, 'session').mockReturnValue(session);
    const spiedSession = jest
      .spyOn(session, 'insertGetId')
      .mockImplementation(async () => 1);
    await connection.insertGetId(
      'insert into "users" ("email") values (?)',
      ['foo'],
      'id',
    );
  ...
  it('Works Statement', async () => {
    ...
    const spiedSession = jest
      ...
      .mockImplementation(async () => true);
    await connection.statement('insert into "users" ("email") values (?)', [
      'foo',
    ]);
    expect(spiedSession).toHaveBeenCalledWith(
      'insert into "users" ("email") values (?)',
