...
import { addslashes, escapeQuoteForSql, isStringable } from '../../utils';
...
class MysqlGrammar extends Grammar {
  ...
  public compileCreateView(
    ...
    const definer = isStringable(registry.definer)
      ? this.getValue(registry.definer).toString()
      : '';
    ...
    if (registry.check ?? false) {
      const checkType = isStringable(registry.checkType)
        ...
        : '';
      sql += ` with${checkType} check option`;
    }
  ...
  protected compileCreateTable(blueprint: BlueprintI): string {
    ...
    const primaryKey = this.getCommandByName<CommandIndexDefinition>(
      blueprint,
      'primary',
    );
    ...
    if (primaryKey) {
      ...
      const algorithm = isStringable(registry.algorithm)
        ...
        : '';
      tableStructure.push(
        `primary key ${algorithm ? 'using ' + algorithm : ''}(${this.columnize(primaryKey.getRegistry().columns)})`,
      );
  ...
  protected compileCreateEncoding(
    ...
    const charset =
      blueprint.getRegistry().charset ?? connection.getConfig('charset');

    // First we will set the character set if one has been set on either the create
    // blueprint itself or on the root configuration for the connection that the
    // table is being created on. We will add these to the create table query.
    if (isStringable(charset)) {
      sql += ` default character set ${this.getValue(charset).toString()}`;
    }

    const collation =
      blueprint.getRegistry().collation ?? connection.getConfig('collation');
  ...
  protected compileCreateEngine(
    ...
    const engine =
      blueprint.getRegistry().engine ?? connection.getConfig('engine');
    if (isStringable(engine)) {
  ...
  public compileChange(blueprint: BlueprintI): string {
    ...
    for (const column of blueprint.getChangedColumns()) {
      ...
      const renameTo = this.getValue(registry.renameTo ?? '').toString();
  ...
  public compileTableComment(
    ...
    return `alter table ${this.wrapTable(blueprint)} comment = '${escapeQuoteForSql(command.getRegistry().comment)}'`;
  }
  ...
  protected compileTypeDateTime(column: ColumnDefinition): string {
    const registry = column.getRegistry();
    const current =
      (registry.precision ?? 0) > 0
        ? `CURRENT_TIMESTAMP(${registry.precision})`
        : 'CURRENT_TIMESTAMP';
    ...
    if (registry.useCurrentOnUpdate ?? false) {
  ...
  protected compileModifyAfter(
    ...
    const after = this.getValue(column.getRegistry().after ?? '').toString();
    if (after) {
  ...
  protected compileModifyCharset(
    ...
    const charset = this.getValue(
      column.getRegistry().charset ?? '',
    ).toString();
    if (charset) {
  ...
  protected compileModifyCollate(
    ...
    const collation = this.getValue(
      column.getRegistry().collation ?? '',
    ).toString();
    if (collation) {
  ...
  protected compileModifyFirst(
    ...
    if (column.getRegistry().first ?? false) {
      return ' first';
    }
  ...
  protected compileModifyIncrement(
    ...
    if (
      (column.getRegistry().autoIncrement ?? false) &&
      this.serials.includes(column.getRegistry().type)
    ) {
  ...
  protected compileModifyInvisible(
    ...
    if (column.getRegistry().invisible ?? false) {
      return ' invisible';
    }
  ...
  protected compileModifyNullable(
    ...
    if (
      !this.needColumnAs(column.getRegistry().virtualAs) &&
      !this.needColumnAs(column.getRegistry().storedAs)
    ) {
  ...
  protected compileModifySrid(
    ...
    const srid = column.getRegistry().srid ?? 0;
    if (srid > 0) {
      return ` srid ${srid}`;
    }
    return '';
  ...
  protected compileModifyUnsigned(
    ...
    if (column.getRegistry().unsigned ?? false) {
      return ' unsigned';
    }
