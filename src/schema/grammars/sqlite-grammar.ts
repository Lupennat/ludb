...
class SqliteGrammar extends Grammar {
  ...
  protected addForeignKeys(blueprint: BlueprintI): string {
    const foreigns = this.getCommandsByName<CommandForeignKeyDefinition>(
      blueprint,
      'foreign',
    );
    ...
    return foreigns.reduce((sql, command) => {
      ...
      if (isStringable(onDelete)) {
        sql += ` on delete ${this.getValue(onDelete).toString()}`;
      }

      if (isStringable(onUpdate)) {
  ...
  protected addPrimaryKeys(blueprint: BlueprintI): string {
    const primary = this.getCommandByName<CommandIndexDefinition>(
      blueprint,
      'primary',
    );
  ...
  protected compileModifyNullable(
    ...
    if (
      column.getRegistry().virtualAs === undefined &&
      column.getRegistry().storedAs === undefined
    ) {
      return (column.getRegistry().nullable ?? false) ? '' : ' not null';
    }
