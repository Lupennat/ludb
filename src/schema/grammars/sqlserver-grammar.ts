...
class SqlserverGrammar extends Grammar {
  ...
  public compileCreateType(
    ...
    if (isStringable(definition)) {
      return `create type ${this.getValue(name).toString()} external name ${this.getValue(definition).toString()}`;
    }
  ...
  public compileChange(blueprint: BlueprintI): string | string[] {
    ...
    for (const column of blueprint.getChangedColumns()) {
      let sql = `alter table ${this.wrapTable(blueprint)} alter column ${this.wrap(column)} ${this.getType(column)}`;
   ...
   */
  protected compileDropDefaultConstraint(
    blueprint: BlueprintI,
    columns: Stringable[],
  ): string {
  ...
  public compileDropTableIfExists(blueprint: BlueprintI): string {
    const table = escapeQuoteForSql(
      `${this.getTablePrefix()}${this.getValue(blueprint.getTable()).toString()}`,
    );
  ...
  protected compileTypeChar(column: ColumnDefinition): string {
    return (column.getRegistry().length ?? 0)
      ? `nchar(${column.getRegistry().length})`
      : 'nchar';
  }
  ...
  protected compileTypeString(column: ColumnDefinition): string {
    return (column.getRegistry().length ?? 0)
      ? `nvarchar(${column.getRegistry().length})`
      : 'nvarchar';
  }
  ...
  protected compileTypeTimestamp(column: ColumnDefinition): string {
    if (column.getRegistry().useCurrent ?? false) {
  ...
  protected compileModifyPersisted(
    ...
    if (registry.change ?? false) {
      if (registry.type === 'computed') {
        return (registry.persisted ?? false)
          ? ' add persisted'
          : ' drop persisted';
      }
