...
class PostgresGrammar extends Grammar {
  ...
  public compileCreateType(
    name: Stringable,
    type: 'range',
    definition: PostgresRangeType,
  ): string;
  public compileCreateType(
    name: Stringable,
    type: 'array',
    definition: PostgresArrayType[],
  ): string;
  public compileCreateType(
    name: Stringable,
    type: 'fn',
    definition: PostgresFunctionType,
  ): string;
  public compileCreateType(
    name: Stringable,
    type: 'domain',
    definition: PostgresDomainType,
  ): string;
  public compileCreateType(
    name: Stringable,
  ...
  public compileCreateType(
    ...
    switch (type) {
      case 'fn':
        return this.createTypeFunction(
          name,
          definition as PostgresFunctionType,
        );
      case 'domain':
  ...
  protected createTypeArray(
    name: Stringable,
    definition: PostgresArrayType[],
  ): string {
    const arrayOptions: string[] = [];
    for (const element of definition) {
      const collation = element.collation ?? '';
      arrayOptions.push(
        `${element.name} ${element.type}${collation !== '' ? ' collate ' + this.wrap(collation) : ''}`,
      );
    }
  ...
  protected createTypeFunction(
    name: Stringable,
    definition: PostgresFunctionType,
  ): string {
    const fnOptions: string[] = [];
    for (const [key, value] of Object.entries(definition)) {
      const keyType = key as keyof PostgresFunctionType;
      let valueType = value as PostgresFunctionType[typeof keyType];

      switch (true) {
        case keyType === 'passedbyvalue':
          valueType = value as boolean | undefined;
          if (valueType ?? false) {
            fnOptions.push(keyType.toUpperCase());
          }
          break;
        case keyType === 'collatable' || keyType === 'preferred':
          valueType = value as boolean | undefined;
          fnOptions.push(
            `${keyType.toUpperCase()} = ${(valueType ?? false) ? 'true' : 'false'}`,
          );
          break;
        case keyType === 'delimiter':
          valueType = value as string | undefined;
          if (valueType ?? '') {
            fnOptions.push(
              `${keyType.toUpperCase()} = ${this.quoteString(valueType!)}`,
            );
          }
          break;
        default:
          valueType = value != null ? (value as string | number) : '';
          if (valueType !== '') {
   ...
   */
  public compileCreateView(
    name: Stringable,
    command?: CommandViewDefinition<ViewRegistryI>,
  ): string {
    if (!command) {
    ...
    let sql = `create${registry.temporary ? ' temporary' : ''}${
      (registry.recursive ?? false) ? ' recursive' : ''
    ...
    const viewAttribute = isStringable(registry.viewAttribute)
      ? this.getValue(registry.viewAttribute).toString()
      : '';
    ...
    if (registry.check ?? false) {
      const checkType = isStringable(registry.checkType)
        ? this.getValue(registry.checkType).toString()
        : '';
  ...
  public compileChange(blueprint: BlueprintI): string {
    ...
    for (const column of blueprint.getChangedColumns()) {
      const changes = [
        `type ${this.getType(column)}${this.compileModifyCollate(blueprint, column)}`,
      ];
      ...
      columns.push(
        this.prefixArray(`alter column ${this.wrap(column)}`, changes).join(
          ', ',
        ),
      );
    }
  ...
  public compileDropColumn(
    ...
    const columns = this.prefixArray(
      'drop column',
      this.wrapArray(command.getRegistry().columns),
    );
  ...
  public compileAutoIncrementStartingValues(
    ...
    if ((column.autoIncrement ?? false) && value !== undefined) {
  ...
  public compileComment(
    ...
    const change = column.getRegistry().change ?? false;
  ...
  public compileUnique(
    ...
    if (
      (registry.deferrable ?? false) &&
      registry.initiallyImmediate !== undefined
    ) {
      sql += registry.initiallyImmediate
        ? ' initially immediate'
        : ' initially deferred';
    }
  ...
  public compileIndex(
    ...
    const registry = command.getRegistry();
    const algorithm = isStringable(registry.algorithm)
      ? this.getValue(registry.algorithm).toString()
      : '';
  ...
  public compileFulltext(
    ...
    const registry = command.getRegistry();
    const language =
      this.getValue(registry.language ?? '').toString() || 'english';
  ...
  protected compileTypeBigInteger(column: ColumnDefinition): string {
    return (column.getRegistry().autoIncrement ?? false) &&
      !this.needColumnAs(column.getRegistry().generatedAs)
      ? 'bigserial'
      : 'bigint';
  }
   ...
   */
  protected compileModifyCollate(
    _blueprint: BlueprintI,
    column: ColumnDefinition,
  ): string {
    const collation = column.getRegistry().collation;
    if (isStringable(collation)) {
   ...
   */
  protected compileModifyDefault(
    _blueprint: BlueprintI,
    column: ColumnDefinition,
  ): string {
    ...
    if (change) {
      return defaultVal === undefined
        ? 'drop default'
        : `set default ${this.getDefaultValue(defaultVal)}`;
    }
   ...
   */
  protected compileModifyIncrement(
    _blueprint: BlueprintI,
    column: ColumnDefinition,
  ): string {
    const change = column.getRegistry().change ?? false;
    const type = column.getRegistry().type;
    const autoIncrement = column.getRegistry().autoIncrement ?? false;

    if (
      !change &&
      autoIncrement &&
      (this.serials.includes(type) ||
        this.needColumnAs(column.getRegistry().generatedAs))
    ) {
      return ' primary key';
    }
   ...
   */
  protected compileModifyNullable(
    _blueprint: BlueprintI,
    column: ColumnDefinition,
  ): string {
    if (column.getRegistry().change ?? false) {
      return (column.getRegistry().nullable ?? false)
        ? 'drop not null'
        : 'set not null';
    }

    return (column.getRegistry().nullable ?? false) ? ' null' : ' not null';
  }

  /**
   * Get the SQL for a stored as column modifier.
   */
  protected compileModifyStoredAs(
    _blueprint: BlueprintI,
    column: ColumnDefinition,
  ): string {
   ...
   */
  protected compileModifyVirtualAs(
    _blueprint: BlueprintI,
    column: ColumnDefinition,
  ): string {
    ...
    const change = column.getRegistry().change ?? false;
    if (change) {
      if (this.needColumnAs(virtualAs)) {
        throw new Error(
          'This database driver does not support modifying generated columns.',
        );
      }
   ...
   */
  protected compileModifyGeneratedAs(
    _blueprint: BlueprintI,
    column: ColumnDefinition,
  ): string {
    const generatedAs = column.getRegistry().generatedAs;
    if (generatedAs !== undefined) {
      return ` generated ${(column.getRegistry().always ?? false) ? 'always' : 'by default'} as identity${
  ...
  protected needColumnAs(columnAs?: boolean | null | Stringable): boolean {
    if (typeof columnAs === 'boolean') {
      return columnAs;
    }

    return this.getValue(columnAs ?? '').toString() !== '';
  }
}
