...
class PostgresBuilder extends QueryBuilder<
  ConnectionSessionI<PostgresConnection>
> {
  /**
  ...
  public async createDatabase(name: string): Promise<boolean> {
    return await this.getConnection().statement(
      this.getGrammar().compileCreateDatabase(name, this.connection),
    );
  }
  ...
  public async createType(
    ...
    return await this.getConnection().statement(
      this.getGrammar().compileCreateType(name, type, definition),
    );
  }
  ...
  public async getTables(): Promise<PostgresTableDictionary[]> {
    return await this.getTablesFromDatabase<PostgresTableDictionary>(
  ...
  public async getViews(): Promise<PostgresViewDictionary[]> {
    return await this.getViewsFromDatabase<PostgresViewDictionary>(
      this.getConnection().getDatabaseName(),
    );
  }
  ...
  public async getTypes(): Promise<PostgresTypeDictionary[]> {
    const types = await this.getTypesFromDatabase<PostgresTypeDefinition>(
      this.getConnection().getDatabaseName(),
    );

    return types.map<PostgresTypeDictionary>(
      (typeObj: PostgresTypeDefinition) => {
  ...
  public async dropDatabaseIfExists(name: string): Promise<boolean> {
    return await this.getConnection().statement(
      this.getGrammar().compileDropDatabaseIfExists(name),
    );
  }
  ...
  public async dropView(name: Stringable): Promise<boolean> {
    return await this.getConnection().statement(
      this.getGrammar().compileDropView(name),
    );
  }
  ...
  public async dropViewIfExists(name: Stringable): Promise<boolean> {
    return await this.getConnection().statement(
      this.getGrammar().compileDropViewIfExists(name),
    );
  }
  ...
  public async dropType(name: Stringable): Promise<boolean> {
    return await this.getConnection().statement(
      this.getGrammar().compileDropType(name),
    );
  }
  ...
  public async dropTypeIfExists(name: Stringable): Promise<boolean> {
    return await this.getConnection().statement(
      this.getGrammar().compileDropTypeIfExists(name),
    );
  }
  ...
  public async dropDomain(name: Stringable): Promise<boolean> {
    return await this.getConnection().statement(
      this.getGrammar().compileDropDomain(name),
    );
  }
  ...
  public async dropDomainIfExists(name: Stringable): Promise<boolean> {
    return await this.getConnection().statement(
      this.getGrammar().compileDropDomainIfExists(name),
    );
  }
  ...
  public async dropTables(): Promise<void> {
    ...
    for (const dbTable of dbTables) {
      ...
      const tablesToMatch = this.getGrammar().escapeNames([
        dbTable.name,
        qualifiedname,
      ]);

      if (
        tablesToMatch.filter((value) => excludedTables.includes(value))
          .length === 0 &&
    ...
    if (tables.length > 0) {
      this.getConnection().statement(
        this.getGrammar().compileDropTables(tables),
      );
    }
  }
  ...
  public async dropViews(): Promise<void> {
    ...
    const excludedViews = this.getGrammar().escapeNames(
      this.getConnection().getConfig('dont_drop', [
        'geography_columns',
        'geometry_columns',
      ]),
    );
    ...
    for (const dbView of dbViews) {
      ...
      const viewsToMatch = this.getGrammar().escapeNames([
        dbView.name,
        qualifiedname,
      ]);

      if (
        viewsToMatch.filter((value) => excludedViews.includes(value)).length ===
          0 &&
  ...
  public async dropTypes(): Promise<void> {
    ...
    for (const dbType of dbTypes) {
      if (
        !dbType.implicit &&
        schemas.includes(this.getGrammar().escapeNames([dbType.schema])[0])
      ) {
    ...
    if (types.length > 0) {
      await this.getConnection().statement(
        this.getGrammar().compileDropTypes(types),
      );
    }

    if (domains.length > 0) {
      await this.getConnection().statement(
        this.getGrammar().compileDropDomains(domains),
      );
    }
  }
  ...
  protected getSchemas(): string[] {
    return this.parseSearchPath(
      this.getConnection().getConfig<string>('search_path') ||
  ...
  public async getColumns(table: string): Promise<PostgresColumnDictionary[]> {
    ...
    const columns = await this.getColumnsFromDatabase<PostgresColumnDefinition>(
      realTable,
      schema,
    );

    return columns.map<PostgresColumnDictionary>(
      (column: PostgresColumnDefinition) => {
        const { nullable, collation, comment, ...keys } = column;
        const autoincrement =
          keys.default !== null &&
          typeof keys.default === 'string' &&
          keys.default.startsWith('nextval(');
  ...
  public async getIndexes(table: string): Promise<PostgresIndexDictionary[]> {
    ...
    const indexes = await this.getIndexesFromDatabase<PostgresIndexDefinition>(
      realTable,
      schema,
    );

    return indexes.map<PostgresIndexDictionary>(
   ...
   */
  public async getForeignKeys(
    table: string,
  ): Promise<PostgresForeignKeyDictionary[]> {
    const [schema, realTable] = this.parseSchemaAndTable(table);

    const foreignKeys =
      await this.getForeignKeysFromDatabase<PostgresForeignKeyDefinition>(
        realTable,
        schema,
      );

    return foreignKeys.map<PostgresForeignKeyDictionary>(
      (foreignKey: PostgresForeignKeyDefinition) => {
        const { columns, foreign_columns, on_update, on_delete, ...keys } =
          foreignKey;
        const onUpdate =
          {
            a: 'no action',
  ...
  protected parseSearchPath(searchPath: string | string[]): string[] {
    ...
    return searchPath.map((schema) => {
      return schema === '$user'
        ? this.getConnection().getConfig('username', '')
        : schema;
    });
  }
}
