...
class SqliteBuilder extends QueryBuilder<ConnectionSessionI<SqliteConnection>> {
  ...
  public async getTables(): Promise<SqliteTableDictionary[]> {
    ...
    try {
      withSize =
        (await this.getConnection().scalar(
          this.getGrammar().compileDbstatExists(),
        )) ?? false;
    } catch (error) {
      //
    }
  ...
  public async getColumns(table: string): Promise<SqliteColumnDictionary[]> {
    const columns =
      await this.getColumnsFromDatabase<SqliteColumnDefinition>(table);
    ...
    return columns.map<SqliteColumnDictionary>(
      (column: SqliteColumnDefinition) => {
        ...
        return {
          ...
          auto_increment:
            hasPrimaryKey && primary === 1 && lowerType === 'integer',
        };
      },
    );
  }
  ...
  public async getIndexes(table: string): Promise<SqliteIndexDictionary[]> {
    ...
    indexes = indexes.map<SqliteIndexDictionary>(
      (index: SqliteIndexDefinition) => {
  ...
  public async getForeignKeys(
    table: string,
  ): Promise<SqliteForeignKeyDictionary[]> {
    const foreignKeys =
      await this.getForeignKeysFromDatabase<SqliteForeignKeyDefinition>(table);

    return foreignKeys.map<SqliteForeignKeyDictionary>(
      (foreignKey: SqliteForeignKeyDefinition) => {
  ...
  public async dropTables(): Promise<void> {
    ...
    await this.getConnection().statement(
      this.getGrammar().compileEnableWriteableSchema(),
    );
    await this.getConnection().statement(this.getGrammar().compileDropTables());
    await this.getConnection().statement(
      this.getGrammar().compileDisableWriteableSchema(),
    );
