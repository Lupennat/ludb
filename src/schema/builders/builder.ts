...
> implements SchemaBuilderI<Session>
{
  ...
  constructor(protected connection: Session) {
    this.grammar = connection.getSchemaGrammar() as ReturnType<
      Session['getSchemaGrammar']
    >;
  }
  ...
  public async hasColumn(table: string, column: string): Promise<boolean> {
    return (await this.getColumnListing(table))
  ...
  public async hasColumns(table: string, columns: string[]): Promise<boolean> {
    const tableColumns = (await this.getColumnListing(table)).map((column) =>
      column.toLowerCase(),
    );
   ...
   */
  public async whenTableHasColumn(
    table: string,
    column: string,
   ...
   */
  public async whenTableDoesntHaveColumn(
    table: string,
    column: string,
    callback: BlueprintCallback,
  ): Promise<void> {
  ...
  public async enableForeignKeyConstraints(): Promise<boolean> {
    return await this.getConnection().statement(
      this.getGrammar().compileEnableForeignKeyConstraints(),
    );
  }
  ...
  public async disableForeignKeyConstraints(): Promise<boolean> {
    return await this.getConnection().statement(
      this.getGrammar().compileDisableForeignKeyConstraints(),
    );
  }
   ...
   */
  public async withoutForeignKeyConstraints<T>(
    callback: () => T,
  ): Promise<T | void> {
  ...
  public getGrammar(): ReturnType<
    ReturnType<this['getConnection']>['getSchemaGrammar']
  > {
    return this.grammar as ReturnType<
      ReturnType<this['getConnection']>['getSchemaGrammar']
    >;
  }
   ...
   */
  protected async getViewsFromDatabase<T = ViewDictionary>(
    databaseOrSchema?: string,
  ): Promise<T[]> {
    return this.getConnection().selectFromWriteConnection<T>(
      this.getGrammar().compileViews(databaseOrSchema),
    );
  }
   ...
   */
  protected async getTypesFromDatabase<T = TypeDictionary>(
    databaseOrSchema?: string,
  ): Promise<T[]> {
    return this.getConnection().selectFromWriteConnection<T>(
      this.getGrammar().compileTypes(databaseOrSchema),
    );
  }
