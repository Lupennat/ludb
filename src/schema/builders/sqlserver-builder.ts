  ...
  implements SqlserverSchemaBuilderI
{
  ...
  public async getColumns(table: string): Promise<SqlserverColumnDictionary[]> {
    const columns =
      await this.getColumnsFromDatabase<SqlserverColumnDefinition>(table);

    return columns.map<SqlserverColumnDictionary>(
      (column: SqlserverColumnDefinition) => {
        const {
          type_name,
          nullable,
          autoincrement,
        ...
        if (
          ...
          ['float', 'datetime2', 'datetimeoffset', 'time'].includes(type)
        ) {
          type = `${type}(${precision})`;
        }
  ...
  public async getTypes(): Promise<SqlserverTypeDictionary[]> {
    const types = await this.getTypesFromDatabase<SqlserverTypeDefinition>(
      this.getConnection().getDatabaseName(),
    );

    return types.map<SqlserverTypeDictionary>(
      (column: SqlserverTypeDefinition) => {
  ...
  public async getIndexes(table: string): Promise<SqlserverIndexDictionary[]> {
    const indexes =
      await this.getIndexesFromDatabase<SqlserverIndexDefinition>(table);

    return indexes.map<SqlserverIndexDictionary>(
      (index: SqlserverIndexDefinition) => {
        return {
  ...
  public async getForeignKeys(
    table: string,
  ): Promise<SqlserverForeignKeyDictionary[]> {
    const foreignKeys =
      await this.getForeignKeysFromDatabase<SqlserverForeignKeyDefinition>(
        table,
      );

    return foreignKeys.map<SqlserverForeignKeyDictionary>(
      (foreignKey: SqlserverForeignKeyDefinition) => {
  ...
  public async createDatabase(name: string): Promise<boolean> {
    return await this.getConnection().statement(
      this.getGrammar().compileCreateDatabase(name),
    );
  }
  ...
  public async createType(
    name: Stringable,
    type: 'simple',
    definition: SqlserverSimpleType,
  ): Promise<boolean>;
  public async createType(
    name: Stringable,
    type: 'external',
  ...
  public async dropTables(): Promise<void> {
    await this.getConnection().statement(
      this.getGrammar().compileDropForeignKeys(),
    );
