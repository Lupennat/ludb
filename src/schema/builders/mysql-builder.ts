...
class MysqlBuilder
  extends QueryBuilder<ConnectionSessionI<MysqlConnection>>
  implements MysqlSchemaBuilderI
{
  /**
  ...
  public async dropViews(): Promise<void> {
    ...
    if (views.length > 0) {
      await this.getConnection().statement(
        this.getGrammar().compileDropViews(views),
      );
    }
  }
  ...
  public async getTables(): Promise<MysqlTableDictionary[]> {
    return await this.getTablesFromDatabase<MysqlTableDictionary>(
  ...
  public async getViews(): Promise<MysqlViewDictionary[]> {
    return await this.getViewsFromDatabase<MysqlViewDictionary>(
      this.getConnection().getDatabaseName(),
    );
  }
  ...
  public async getColumns(table: string): Promise<MysqlColumnDictionary[]> {
    ...
    return columns.map<MysqlColumnDictionary>(
  ...
  public async getForeignKeys(
    table: string,
  ): Promise<MysqlForeignKeyDictionary[]> {
    const foreignKeys =
      await this.getForeignKeysFromDatabase<MysqlForeignKeyDefinition>(
        table,
        this.getConnection().getDatabaseName(),
      );

    return foreignKeys.map<MysqlForeignKeyDictionary>(
      (foreignKey: MysqlForeignKeyDefinition) => {
