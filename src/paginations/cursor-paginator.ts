...
class CursorPaginator<T>
  extends AbstractPaginator<T, CursorPaginatorOptions>
  implements CursorPaginatorI<T>
{
  protected static currentCursorResolverCallback:
    | ((name: string) => Cursor | null)
    | undefined;
  ...
  public url(cursor?: Cursor): string {
    ...
    return this.generateUrl(
      cursor === undefined ? {} : { [this.options.name]: cursor.encode() },
    );
  }
   ...
   */
  public getParametersForItem(item: T | Objectable<T>): {
    [key: string]: string;
  } {
    return this.options.parameters.reduce((carry, parameter) => {
      ...
      carry[parameter] = this.ensureParameterIsPrimitive(
        parameter in (item as any)
          ? (item as any)[parameter]
          : (item as any)[afterLast(parameter, '.')],
      );
  ...
  public static resolveCurrentCursor(
    name = 'cursor',
    defaultCursor: Cursor | null = null,
  ): Cursor | null {
   ...
   */
  public static currentCursorResolver(
    resolver: (name: string) => Cursor | null,
  ): void {
