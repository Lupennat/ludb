...
class PostgresConnector extends Connector {
  ...
  public connect(config: FlattedConnectionConfig<PostgresConfig>): Pdo {
    ...
    let ssl:
      ...
      cert:
        (config.sslcert ?? '')
          ? readFileSync(config.sslcert!).toString()
          : undefined,
      key:
        (config.sslkey ?? '')
          ? readFileSync(config.sslkey!).toString()
          : undefined,
      ca:
        (config.sslrootcert ?? '')
          ? readFileSync(config.sslrootcert!).toString()
          : undefined,
    };
   ...
   */
  public async configureIsolationLevel(
   ...
   */
  public async configureEncoding(
    connection: PdoConnectionI,
    config: PostgresConfig,
  ): Promise<void> {
    if (config.charset ?? '') {
   ...
   */
  public async configureTimezone(
    connection: PdoConnectionI,
    config: PostgresConfig,
  ): Promise<void> {
    if (config.timezone ?? '') {
   ...
   */
  public async configureSearchPath(
    connection: PdoConnectionI,
    config: PostgresConfig,
  ): Promise<void> {
    ...
    if (search != null) {
      connection.query(
        `set search_path to ${this.quoteSearchPath(parseSearchPath(search))}`,
      );
    }
  }
   ...
   */
  public async configureSynchronousCommit(
    connection: PdoConnectionI,
    config: PostgresConfig,
  ): Promise<void> {
    if (config.synchronous_commit ?? '') {
      connection.query(
        `set synchronous_commit to '${config.synchronous_commit}'`,
      );
    }
  }
}
