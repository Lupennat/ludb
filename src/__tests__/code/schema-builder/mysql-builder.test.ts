...
describe('Mysql Schema Builder Test', () => {
    ...
    it('Works Create View', async () => {
        ...
        jest.spyOn(session, 'statement')
            ...
            .mockImplementationOnce(async (sql, bindings) => {
                expect(sql).toBe(
                    'create algorithm=merge definer=user@table sql security definer view `prefix_foo` (`id`, `name`) as select "id", "name" from "baz" where "type" in (\'bar\', \'bax\') with cascade check option'
                );
                expect(bindings).toBeUndefined();
                return true;
            });
    ...
    it('Works Get Columns', async () => {
        ...
        jest.spyOn(session, 'selectFromWriteConnection')
            .mockImplementationOnce(async (sql, bindings) => {
                expect(sql).toBe(
                    "select column_name as `name`, data_type as `type_name`, column_type as `type`, collation_name as `collation`, is_nullable as `nullable`, column_default as `default`, column_comment as `comment`, extra as `extra` from information_schema.columns where table_schema = 'db' and table_name = 'prefix_table'"
                );
                expect(bindings).toBeUndefined();
                return [{ name: 'column' }];
            })
    ...
    it('Works Get Indexes', async () => {
        ...
        jest.spyOn(session, 'selectFromWriteConnection').mockImplementationOnce(async (sql, bindings) => {
            expect(sql).toBe(
                "select index_name as `name`, group_concat(column_name order by seq_in_index) as `columns`, index_type as `type`, not non_unique as `unique` from information_schema.statistics where table_schema = 'db' and table_name = 'prefix_table' group by index_name, index_type, non_unique"
            );
            expect(bindings).toBeUndefined();
            return [
                {
    ...
    it('Works Get Tables', async () => {
        ...
        jest.spyOn(session, 'selectFromWriteConnection').mockImplementationOnce(async (sql, bindings) => {
            expect(sql).toBe(
                "select table_name as `name`, (data_length + index_length) as `size`, table_comment as `comment`, engine as `engine`, table_collation as `collation` from information_schema.tables where table_schema = 'db' and table_type = 'BASE TABLE' order by table_name"
            );
            expect(bindings).toBeUndefined();
    ...
    it('Works Get Views', async () => {
        ...
        jest.spyOn(session, 'selectFromWriteConnection').mockImplementationOnce(async (sql, bindings) => {
            expect(sql).toBe(
                "select table_name as `name`, view_definition as `definition` from information_schema.views where table_schema = 'db' order by table_name"
            );
            expect(bindings).toBeUndefined();
