...
import { GLOBAL_CACHE, jsonStringify, merge } from './utils';

export default class CacheManager {
  /**
   * map connection with cache driver instances
   */
  protected cacheMap: Record<string, string | symbol> = {};
   ...
   */
  protected caches: Record<string | symbol, CacheDriverI> = {};
  ...
  protected createCaches(config: DBConfig): void {
    ...
    if (allCache.resolver) {
      hasAll = true;
      this.caches[GLOBAL_CACHE] = allCache.resolver();
    ...
    for (const name in config.connections) {
      ...
      if (localCache.resolver) {
        ...
        this.cacheMap[name] = GLOBAL_CACHE;
      }

      this.cacheConfig[name] = merge<Omit<CacheConfiguration, 'resolver'>>(
        allCache,
        localCache,
      );
    }
  ...
  protected generateQueryCache(
    ...
    return {
      ...
      key: `${connectionName}:${prefix !== '' ? prefix + ':' : ''}${
        (queryCacheConnection.key ?? '') !== ''
          ? queryCacheConnection.key + ':'
          : ''
      }${this.generateHash(queryCacheConnection.query, queryCacheConnection.bindings)}`,
      options: merge<Record<string, any>>(
        config.options ?? {},
        queryCacheConnection.options ?? {},
      ),
      time: Date.now(),
    };
  }
   ...
   */
  protected getCacheName(connectionName: string): string | symbol | undefined {
  ...
  public async get<T = any>(
    ...
    const queryCacheManager = this.generateQueryCache(
      connectionName,
      queryCacheConnection,
    );
  ...
  public async terminate(): Promise<void> {
    ...
    if (GLOBAL_CACHE in this.caches) {
      promises.push(this.caches[GLOBAL_CACHE].disconnect());
    }
