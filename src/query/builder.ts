...
class Builder extends CommonQueryBuilder implements BuilderI {
    /**
    ...
    protected async orderedChunkById<T = Dictionary>(
        ...
        do {
            ...
            let results = descending
                ? await clone.forPageBeforeId(count, lastId, column).get<T>()
                : await clone.forPageAfterId(count, lastId, column).get<T>();
    ...
    public lazy<T = Dictionary>(chunkSize = 1000): AsyncGenerator<T> {
        ...
        return async function* (this: BuilderI) {
            let page = 1;
     ...
     */
    protected cloneForPaginationCount(): BuilderI {
    ...
    protected async paginateUsingCursor<T>(
        ...
        if (cursorInstance != null) {
            const addCursorConditions = (builder: QueryBuilderI, previousColumn: null | string, i: number): void => {
     ...
     */
    protected getOriginalColumnNameForCursorPagination(builder: BuilderI, parameter: string): string {
    ...
    public async insertUsing(
        columns: Stringable[],
        query: QueryAbleCallback<BuilderI> | BuilderI | Stringable
    ): Promise<number> {
        this.applyBeforeQueryCallbacks();
