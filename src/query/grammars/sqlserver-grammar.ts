...
class SqlserverGrammar extends Grammar {
  ...
  public compileSelect(query: GrammarBuilderI): string {
    ...
    if ((registry.offset ?? 0) && registry.orders.length === 0) {
  ...
  public prepareBindingForJsonContains(
    ...
    return typeof binding === 'boolean'
      ? JSON.stringify(binding, stringifyReplacer(this))
      : binding;
  }
  ...
  protected compileDeleteWithoutJoins(
    ...
    return limit !== null &&
      limit > 0 &&
      Number(query.getRegistry().offset) <= 0
  ...
  protected compileJsonUpdateForArray(
    ...
    const condition = `(select count(*) from openjson(${this.wrap(column)}, ${searchPath})) >= ${Number(position) + 1}`;

    const expressionPath = `case when ${condition} then ${path} else ${appendPath} end`;
    return this.compileJsonNullPatch(
      this.compileJsonUpdate(expression, expressionPath, value),
      path,
      value,
    );
  }
   ...
   */
  protected compileJsonUpdateForPath(
    expression: string,
    path: string,
    value: any,
  ): string {
    return this.compileJsonNullPatch(
      this.compileJsonUpdate(expression, path, value),
      path,
      value,
    );
  }
   ...
   */
  protected compileJsonNullPatch(
    expression: string,
    path: string,
    value: any,
  ): string {
  ...
  public compileUpsert(
    ...
    const on = uniqueBy
      .map((column) => {
        return `${this.wrap('laravel_source.' + column)} = ${this.wrap(query.getRegistry().from + '.' + column)}`;
      })
      .join(' and ');

    sql += `on ${on} `;

    const stringUpdate = update.filter(
      (binding) => typeof binding === 'string',
    ) as string[];
    const rowValues = update.filter(
      (binding) => typeof binding !== 'string',
    ) as RowValues[];
    ...
    const updateString = stringUpdate
      ...
      .concat(
        rowValues.reduce<string[]>((carry: string[], item) => {
    ...
    return Object.keys(combinedValues).reduce((acc: any[], key: string) => {
      if (!jsonKeys.includes(key)) {
        ...
        for (const subkey in combinedValues[key]) {
          ...
          acc.push(
            this.mustBeJsonStringified(value)
