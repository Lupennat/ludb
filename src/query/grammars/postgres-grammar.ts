...
class PostgresGrammar extends Grammar {
  ...
  public compileUpdate(query: GrammarBuilderI, values: RowValues): string {
    ...
    const limit = query.getRegistry().limit ?? 0;
  ...
  protected compileJsonUpdateColumn(column: string, values: RowValues): string {
    ...
    for (const key in values) {
      ...
      const value = this.isExpression(values[key])
        ? `'${this.parameter(values[key])}'`
        : '?::jsonb';
  ...
  public compileUpdateFrom(query: GrammarBuilderI, values: RowValues): string {
    ...
    const expressionsSql =
      expressions.length > 0
        ? this.compileExpressions(query, expressions) + ' '
        : '';
   ...
   */
  protected compileUpdateWithJoinsOrLimit(
    query: GrammarBuilderI,
    values: RowValues,
  ): string {
    const table = this.wrapTable(query.getRegistry().from);
  ...
  public prepareBindingsForUpdate(
    ...
    if (query.getRegistry().joins.length || (query.getRegistry().limit ?? 0)) {
      return this.mergeBindingsAndValue(
  ...
  public compileTruncate(query: GrammarBuilderI): { [key: string]: Binding[] } {
    return {
      [`truncate ${this.wrapTable(query.getRegistry().from)} restart identity cascade`]:
        [],
    };
  }
