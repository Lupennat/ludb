...
class Grammar extends BaseGrammar {
  ...
  public compileSelect(query: GrammarBuilderI): string {
    return this.wrapSelectExpression(query, () => {
      ...
      if (
        (registry.unions.length > 0 || registry.havings.length > 0) &&
   ...
   */
  protected wrapSelectExpression(
    query: GrammarBuilderI,
    callback: () => string,
  ): string {
    const registry = query.getRegistry();
    const unionExpressionSql =
      registry.unionExpressions.length > 0
        ? this.compileExpressions(query, registry.unionExpressions) + ' '
        : '';
      ...
      switch (key) {
        ...
        case 'recursionLimit':
          if (registry.recursionLimit !== null) {
            sqls.push(
              this.compileRecursionLimit(query, registry.recursionLimit),
            );
          }
          break;
      }
    ...
    for (const expression of expressions) {
      const columns =
        expression.columns.length > 0
          ? `(${this.columnize(expression.columns)}) `
          : '';
      const materialized =
   ...
   */
  protected compileRecursionLimit(
  ...
  protected recursiveKeyword(expressions: Cte[]): string {
    return expressions.filter((expression) => expression.recursive === true)
      .length > 0
      ? 'recursive '
      : '';
  }
  ...
  protected compileColumns(
    ...
    const select =
      query.getRegistry().distinct !== false ? 'select distinct' : 'select';
  ...
  protected compileWhereRaw(_query: GrammarBuilderI, where: WhereRaw): string {
    return where.sql instanceof ExpressionContract
      ? where.sql.getValue(this).toString()
      : where.sql;
  }
   ...
   */
  protected compileWhereExpression(
    _query: GrammarBuilderI,
   ...
   * Compile a "between" where clause.
   */
  protected compileWhereBetweenColumns(
    _query: GrammarBuilderI,
    where: WhereBetweenColumns,
  ): string {
    const between = where.not ? 'not between' : 'between';
   ...
   */
  protected compileWhereRowValues(
    _query: GrammarBuilderI,
    where: WhereMultiColumn,
  ): string {
   ...
   */
  protected compileWhereJsonBoolean(
    _query: GrammarBuilderI,
    where: WhereBoolean,
  ): string {
   ...
   */
  protected compileWhereJsonContains(
    _query: GrammarBuilderI,
    where: WhereContains,
  ): string {
    const not = where.not ? 'not ' : '';
   ...
   */
  public prepareBindingForJsonContains(
   ...
   */
  protected compileWhereJsonContainsKey(
    _query: GrammarBuilderI,
    where: WhereKeyContains,
  ): string {
    const not = where.not ? 'not ' : '';
   ...
   */
  protected compileWhereJsonLength(
    _query: GrammarBuilderI,
    where: WhereLength,
  ): string {
    const not = where.not ? 'not ' : '';
   ...
   */
  protected compilewhereFulltext(
    query: GrammarBuilderI,
    where: whereFulltext,
  ): string {
    const not = where.not ? 'not ' : '';
   ...
   */
  protected compileHavingRaw(
    _query: GrammarBuilderI,
    having: HavingRaw,
  ): string {
    return having.sql instanceof ExpressionContract
      ? having.sql.getValue(this).toString()
      : having.sql;
  }
   ...
   */
  protected compileHavingExpression(
    _query: GrammarBuilderI,
   ...
   */
  protected compileHavingBetween(
    _query: GrammarBuilderI,
    having: HavingBetween,
  ): string {
   ...
   */
  protected compileHavingBitwise(
    query: GrammarBuilderI,
    having: HavingBasic,
  ): string {
    return this.compileHavingBasic(query, having);
  ...
  public compileInsertUsing(
    ...
    return this.wrapInsertUsingExpression(query, sql, (sql) => {
      ...
      if (
        columns.length === 0 ||
        columns.filter(
          (column) => !['*'].includes(this.getValue(column).toString()),
        ).length === 0
      ) {
  ...
  protected wrapInsertUsingExpression(
    ...
    const expressionSql =
      expressions.length > 0
        ? this.compileExpressions(query, query.getRegistry().expressions) + ' '
        : '';
    ...
    const recursionLimitSql =
      recursionLimit !== null
        ? ' ' + this.compileRecursionLimit(query, recursionLimit)
        : '';
   ...
   */
  protected prepareBindingsForMerge(
    _query: GrammarBuilderI,
    bindings: BindingTypes,
  ): Binding[][] {
   ...
   */
  protected prepareValuesForMerge(
    _query: GrammarBuilderI,
    values: RowValues,
  ): any[] {
   ...
   * Prepare the bindings for an update statement.
   */
  public prepareBindingsForUpdateFrom(
    _query: GrammarBuilderI,
    _bindings: BindingTypes,
    _values: RowValues,
  ): any[] {
    throw new Error(
      'This database engine does not support the updateFrom method.',
   ...
   */
  protected wrapDeleteExpression(
    query: GrammarBuilderI,
    callback: () => string,
  ): string {
    const expressions = query.getRegistry().expressions;
  ...
  public compileTruncate(query: GrammarBuilderI): { [key: string]: Binding[] } {
    return {
      [`truncate table ${this.wrapTable(query.getRegistry().from)}`]: [],
    };
  }
  ...
  public substituteBindingsIntoRawSql(
    sql: string,
    bindings:
      | BindingExclude<ExpressionContract>[]
      | BindingExcludeObject<ExpressionContract>,
  ): string {
