...
class BaseQueryBuilder
  extends CommonGrammarBuilder
  implements BaseQueryBuilderI
{
  /**
  ...
  protected async orderedChunkById<T = Dictionary>(
    ...
    do {
      ...
      lastId = results[results.length - 1][alias as keyof T] as
        | number
        | bigint
        | null
        | undefined;
    ...
    return async function* (this: QueryBuilderI) {
      ...
      while (true) {
        ...
        const results = descending
          ? await clone
              .forPageBeforeId(chunkSize, lastId, columnString)
              .get<T>()
          : await clone
              .forPageAfterId(chunkSize, lastId, columnString)
              .get<T>();
        ...
        lastId = results[results.length - 1][aliasString as keyof T] as
          | number
          | bigint
          | null
          | undefined;
   ...
   */
  public async first<T = Dictionary>(
   ...
   */
  public async sole<T = Dictionary>(
    columns: Stringable | Stringable[] = ['*'],
  ): Promise<T> {
   ...
   */
  public async findOr<T = Dictionary>(
   ...
   */
  public async get<T = Dictionary>(
    columns: Stringable | Stringable[] = ['*'],
  ): Promise<T[]> {
    return await this.onceWithColumns<T>(
  ...
  protected async runSelect<T = Dictionary>(): Promise<T[]> {
    return this.getConnection().select<T>(
      this.toSql(),
      this.getBindings(),
  ...
  public async paginate<T = Dictionary>(
    ...
    const results = total
      ? await this.forPage(page, perPage).get<T>(columns)
      : new Array<T>();
  ...
  protected async runPaginationCountQuery(
    ...
    if (this.registry.groups.length || this.registry.havings.length) {
      ...
      if (
        clone.getRegistry().columns === null &&
        this.registry.joins.length > 0
      ) {
      ...
      return await (this.newQuery() as this)
        .from(
          this.raw(
            `(${clone.toSql()}) as ${this.getGrammar().wrap('aggregate_table')}`,
          ),
        )
        .mergeBindings(clone)
    ...
    return (
      this.cloneWithout(
        this.registry.unions.length
          ? ['orders', 'limit', 'offset']
          : ['columns', 'orders', 'limit', 'offset'],
      ).cloneWithoutBindings(
        this.registry.unions.length ? ['order'] : ['select', 'order'],
      ) as this
    )
      .setAggregate('count', this.withoutSelectAliases(columns))
      .get<{ aggregate: number | string }>();
  ...
  protected async paginateUsingCursor<T>(
    ...
    if (cursorInstance != null) {
      const addCursorConditions = (
        builder: GrammarBuilderI,
        previousColumn: null | string,
        i: number,
      ): void => {
        ...
        if (previousColumn !== null) {
          const originalColumn = this.getOriginalColumnNameForCursorPagination(
            this,
            previousColumn,
          );
    ...
    return this.cursorPaginator<T>(
      await this.get<T>(columns),
      perPage,
      cursorInstance,
      {
        path: CursorPaginator.resolveCurrentPath(),
        name,
        parameters: orders.map((order) =>
  ...
  protected getOriginalColumnNameForCursorPagination(
    ...
    for (const column of columns) {
      ...
      if (position > -1) {
        ...
        if (
          parameter === alias ||
          builder.getGrammar().wrap(parameter) === alias
        ) {
          return original;
        }
      }
  ...
  protected enforceOrderBy(): void {
    if (
      this.registry.orders.length === 0 &&
      this.registry.unionOrders.length === 0
    ) {
      throw new Error(
    ...
    if (key === null) {
      ...
      return queryResult.reduce((carry, item) => {
        ...
        if (Array.isArray(newKey) || Buffer.isBuffer(newKey)) {
          throw new Error(
            `key value [${Array.isArray(newKey) ? 'Array' : 'Buffer'}] is not stringable`,
          );
        }
        carry[newKey === null ? 'null' : newKey.toString()] =
          item[column as keyof Dictionary];
        return carry;
      }, {}) as { [key: string]: T };
    }
  }
  ...
  protected getInteger(value: string | number | bigint): number | bigint {
    return BigInt(value) > Number.MAX_SAFE_INTEGER ||
      BigInt(value) < Number.MIN_SAFE_INTEGER
      ? BigInt(value)
      : Number(value);
  }
  ...
  public async aggregate(
    ...
    const results = await (
      this.cloneWithout(
        this.registry.unions.length > 0 || this.registry.havings.length > 0
          ? []
          : ['columns'],
      ).cloneWithoutBindings(
        this.registry.unions.length > 0 || this.registry.havings.length > 0
   ...
   */
  protected async onceWithColumns<T>(
    columns: Stringable[],
    callback: () => Promise<T[]>,
  ): Promise<T[]> {
  ...
  public async insert(values: RowValues | RowValues[]): Promise<boolean> {
    ...
    return this.getConnection().insert(
      ...
      this.cleanBindings(
        sortedRowValues.map((value) => Object.values(value)).flat(1),
      ),
    );
  }
        ...
        values.map((value) => {
          ...
          if (currentKeys == null) {
            ...
            if (keys.length !== currentKeys.length) {
              ...
              throw new Error(
                `Missing columns [${difference.join(', ')}], please add to each rows.`,
              );
            }
          }
          return row;
  ...
  public async insertUsing(
    ...
    const fullBindings = [
      ...this.registry.bindings['expressions'],
      ...bindings,
    ];
  ...
  public async update(values: RowValues): Promise<number> {
    ...
    return await this.getConnection().update(
      sql,
      this.cleanBindings(
        this.getGrammar().prepareBindingsForUpdate(
          this,
  ...
  public async updateFrom(values: RowValues): Promise<number> {
    ...
    return await this.getConnection().update(
      sql,
      this.cleanBindings(
        this.getGrammar().prepareBindingsForUpdateFrom(
          this,
          this.registry.bindings,
          values,
        ),
   ...
   */
  public async incrementEach(
    columns: NumericValues,
    extra: RowValues = {},
  ): Promise<number> {
    ...
    for (const column in columns) {
      ...
      processed[column] = this.raw(
        `${this.getGrammar().wrap(column)} + ${amount.toString()}`,
      );
    }
   ...
   */
  public async delete(
    id: string | number | bigint | null = null,
  ): Promise<number> {
    ...
    return this.getConnection().delete(
      ...
      this.cleanBindings(
        this.getGrammar().prepareBindingsForDelete(this.registry.bindings),
      ),
    );
  }
  ...
  public async explain(): Promise<string[]> {
    ...
    return await this.getConnection().select<string>(
      `EXPLAIN ${sql}`,
      bindings,
    );
  }
   ...
   */
  protected newJoinClause<T extends GrammarBuilderI>(
    parentQuery: T,
    type: string,
    table: Stringable,
  ): JoinClauseI {
