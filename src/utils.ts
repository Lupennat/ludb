...
export const GLOBAL_CACHE = Symbol('GLOBAL_CACHE');

export function stringifyReplacer(
  grammar: Grammar,
...
function getErrorMessages(error: any, depth: number, current = 1): string[] {
  ...
  if (error.cause != null) {
    messages = messages.concat(
      depth > current
        ? getErrorMessages(error.cause, depth, current + 1)
        : error.cause.message,
    );
  }
  if (error.errors != null && Array.isArray(error.errors)) {
    for (const err of error.errors) {
      messages = messages.concat(
        depth > current
          ? getErrorMessages(err, depth, current + 1)
          : err.message,
      );
    }
  }
...
export function getMessagesFromError(error: any, depth = 1): string[] {
  return getErrorMessages(error, depth).filter(
...
export function jsonParse<T = any>(json: string): T {
  return JSON.parse(json, (_key, value) => {
    switch (true) {
      case value != null && value.type === 'Buffer':
        return Buffer.from(value.data);
      case value != null && value.type === 'BigInt':
        return BigInt(value.data);
      case value != null && value.type === 'TypedBinding':
        return TypedBinding.create(
          value.data.type,
          value.data.value,
          value.data.options,
        );
