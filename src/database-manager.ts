...
class DatabaseManager<const Config extends DBConfig> {
  ...
  public connection<const T extends keyof Config['connections'] & string>(
    ...
    return this.connections[name] as ExtractConnection<
      Config['connections'][T]
    >;
  }
  ...
  protected createConnection<
    const T extends keyof Config['connections'] & string,
  >(name: T): any {
    ...
    if (!this.availableDrivers().includes(driver)) {
      try {
        ...
        throw new Error(
          `Lupdo driver is missing, please install driver for "${driver}"`,
        );
      }
    }
   ...
   */
  public async purge<const T extends keyof Config['connections'] & string>(
    name: T,
  ): Promise<void> {
   ...
   */
  public async disconnect<const T extends keyof Config['connections'] & string>(
    name: T,
  ): Promise<void> {
    if (name in this.connections) {
  ...
  public async reconnect<const T extends keyof Config['connections'] & string>(
    ...
    return this.connections[name].reconnect() as Promise<
      ExtractConnection<Config['connections'][T]>
    >;
  }
  ...
  public availableDrivers(): string[] {
    ...
    return this.supportedDrivers().filter((value) =>
      pdoDrivers.includes(value),
    );
  }
