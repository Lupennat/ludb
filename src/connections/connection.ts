...
abstract class Connection<
  const Config extends ConnectionConfig = ConnectionConfig,
> implements DriverConnectionI
{
  /**
   ...
   */
  public abstract getSchemaBuilder(): SchemaBuilderI<
    ConnectionSessionI<DriverConnectionI>
  >;
   ...
   */
  protected createSingleConnection(
    config: FlattedConnectionConfig<Config>,
  ): void {
  ...
  protected getReadWriteConfig(
    ...
    | FlattedConnectionConfig<ReadAndWriteConnectionOptions<Config>>
    | undefined {
    if (type in this.config) {
      const options = this.config[type];
      return options as FlattedConnectionConfig<
        ReadAndWriteConnectionOptions<Config>
      >;
    }
    return undefined;
  }
   ...
   */
  protected createPdoSchemaResolver(
    config: FlattedConnectionConfig<Config>,
  ): Pdo {
  ...
  protected bindValue(
    ...
    if (binding instanceof ExpressionContract) {
      throw new Error(
        'Expression binding can not be binded directly to statement.',
      );
    }
  ...
  public prepareBindings(
    ...
    if (Array.isArray(bindings)) {
      ...
      return Object.keys(bindings).reduce(
        (carry: BindingExcludeObject<ExpressionContract>, key) => {
  ...
  public async disconnect(): Promise<void> {
    const promises = [
      this.getPdo().disconnect(),
      this.getSchemaPdo().disconnect(),
    ];
    if (this.readPdo !== null) {
   ...
   */
  public listen(
    callback: (event: QueryExecuted) => void | Promise<void>,
  ): void {
   ...
   */
  public table(
    table: QueryAbleCallback<QueryBuilderI> | QueryBuilderI | Stringable,
    as?: string,
  ): QueryBuilderI {
   ...
   */
  public useWriteConnectionWhenReading(
    value?: boolean,
  ): ConnectionSessionI<this> {
