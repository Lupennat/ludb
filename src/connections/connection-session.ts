...
class ConnectionSession<
  DriverConnection extends DriverConnectionI = DriverConnectionI,
> implements ConnectionSessionI<DriverConnection>
{
  ...
  public async scalar<T>(
    ...
    const record = await this.selectOne<Dictionary>(
      query,
      bindings,
      useReadPdo,
    );
  ...
  public async select<T = Dictionary>(
    ...
    return await this.run<T[]>(query, bindings, async (query, bindings) => {
      ...
      const cache = await this.getCacheManager()?.get<T[]>(
      ...
      if (cache) {
        await this.getCacheManager()?.store<T[]>(
          this.getDriverConnection().getName(),
          {
            ...cache,
            result,
  ...
  public async cursor<T = Dictionary>(
    ...
    const statement = await this.run<
      PdoPreparedStatementI | PdoTransactionPreparedStatementI | never[]
    >(query, bindings, async (query, bindings) => {
      if (this.pretending()) {
        return [];
      }
  ...
  public async insertGetId<T = number | bigint | string>(
    ...
    return this.run<T | null>(query, bindings, async (query, bindings) => {
      ...
      const id = (await statement.lastInsertId(sequence)) as T;
  ...
  public async pretend(
    callback: (session: this) => void | Promise<void>,
  ): Promise<LoggedQuery[]> {
   ...
   */
  public async withoutPretending<T>(
  ...
  protected async run<T>(
    ...
    try {
      ...
      try {
        result = await this.handleQueryError<T>(
          error as QueryError,
          query,
          bindings,
          callback,
  ...
  protected logQuery(
    ...
    if (this.loggingQueries) {
      this.queryLog.push({
        query: this.getQueryGrammar().substituteBindingsIntoRawSql(
          query,
  ...
  public async transaction(
    ...
    for (let currentAttempt = 1; currentAttempt <= attempts; currentAttempt++) {
      ...
      try {
        ...
        await this.handleCommitTransactionError(
          error,
          currentAttempt,
          attempts,
        );
   ...
   */
  protected async handleTransactionError(
    error: Error,
  ...
  protected async createTransaction(): Promise<void> {
    if (this.transactionLevel() === 0) {
      ...
      this.getQueryGrammar().supportsSavepoints()
    ) {
  ...
  protected async createSavepoint(): Promise<void> {
    await this.getEnsuredPdoTransaction().exec(
      this.getQueryGrammar().compileSavepoint(
        `trans${this.transactionLevel() + 1}`,
      ),
    );
  }
  ...
  protected getElapsedTime(start: number): number {
    return (
      Math.round(((Date.now() - start) / 1000 + Number.EPSILON) * 100) / 100
    );
  }
  ...
  protected getEnsuredPdo(): Pdo {
    return this.isSchemaConnection
      ? this.getSchemaPdo()
      : this.getDriverConnection().getPdo();
  }
  ...
  public getReadPdo(): Pdo | PdoTransactionI {
    ...
    return this.isSchemaConnection
      ? this.getSchemaPdo()
      : this.getDriverConnection().getReadPdo();
  }
  ...
  public getConfig<T>(option?: string, defaultValue?: T): T {
    ...
    return this.getDriverConnection().getConfig<T>(
      option as any,
      defaultValue as any,
    );
  }
  ...
  public getSchemaGrammar(): ReturnType<DriverConnection['getSchemaGrammar']> {
    return this.getDriverConnection().getSchemaGrammar() as ReturnType<
      DriverConnection['getSchemaGrammar']
    >;
  }
  ...
  public getQueryGrammar(): ReturnType<DriverConnection['getQueryGrammar']> {
    return this.getDriverConnection().getQueryGrammar() as ReturnType<
      DriverConnection['getQueryGrammar']
    >;
  }
